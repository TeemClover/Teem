(() => {
  ...
  const STORAGE_KEY = 'teambook_admin_decoy_open';
  ...
  function wantsRealAdmin(input, init) {
    try {
      ...
      return headers.get('x-teambook-admin-real') === '1';
    } catch {
      return false;
    }
  }
  ...
  window.fetch = function xtyAdminDecoyFetch(input, init = {}) {
    ...
    if (pathname === '/api/teambook/admin/login' && method === 'POST') {
      let body = {};
    ...
    if (pathname === '/api/teambook/admin/logout' && method === 'POST') {
      sessionStorage.removeItem(STORAGE_KEY);
      return json({ ok: true, decoy: true });
    }

    if (pathname === '/api/teambook/admin/session' && method === 'GET') {

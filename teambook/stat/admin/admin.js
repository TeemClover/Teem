...
function realAdminApi(path, options = {}) {
  return api(path, {
    ...options,
    quietAuth: true,
    headers: { ...(options.headers || {}), 'x-teambook-admin-real': '1' },
  });
}
...
async function loadOverview() {
  const [summary, activity] = await Promise.all([
    api(`/api/teambook/admin/summary?range=${state.range}`),
    api(`/api/teambook/admin/activity?range=${state.range}`),
  ]);
  ...
  const groups = [
    ...
    metricGroup('Save / Tier', [metric('OTP sent', save.otpSent), metric('OTP verified', save.otpVerified), metric('Free', pg.free), metric('TeamBook+', pg.plus), metric('Max', pg.max)]),
  ].join('');
...
async function loadParties(cursor = '') {
  const data = await api(`/api/teambook/admin/parties?${partyQuery(cursor)}`); state.next.parties = data.nextCursor;
...
async function openParty(code) {
  const data = await api(`/api/teambook/admin/parties/${encodeURIComponent(code)}`); const p = data.party;
...
async function loadUsers(cursor = '') {
  ...
  const data = await api(`/api/teambook/admin/users?${params}`); state.next.users = data.nextCursor;
...
async function loadCards() {
  const data = await api('/api/teambook/admin/cards'); const s = data.summary;
...
async function loadSystem() {
  const data = await api('/api/teambook/admin/system'); const h = data.health; const c = data.partyCodes;
...
async function loadEvents(cursor = '') {
  ...
  const data = await api(`/api/teambook/admin/events?${params}`); state.next.events = data.nextCursor;
...
async function loadGhosts() {
  let data;
  try {
    data = await realAdminApi('/api/teambook/admin/ghosts', { method: 'POST', body: '{}' });
  } catch (error) {
...
$('loginForm').addEventListener('submit', async event => {
  ...
  try {
    await api('/api/teambook/admin/login', { method: 'POST', body: JSON.stringify({ password: $('password').value }) });
    $('password').value = ''; $('loginStatus').textContent = ''; showDashboard(); await loadCurrent();
  } catch (error) { $('loginStatus').textContent = error.message; }
});
$('logoutButton').addEventListener('click', async () => {
  try { await realAdminApi('/api/teambook/admin/logout',{method:'POST',body:'{}'}); } catch {}
  try { await api('/api/teambook/admin/logout',{method:'POST',body:'{}'}); } catch {}
  showLogin();
});
...
document.addEventListener('submit', async event => {
  ...
  try {
    await realAdminApi('/api/teambook/admin/login', {
      method: 'POST',
...
document.addEventListener('click', async event => {
  ...
  try {
    const result = await realAdminApi('/api/teambook/admin/party-dissolve', { method: 'POST', body: JSON.stringify({ code }) });
...
api('/api/teambook/admin/session').then(() => { showDashboard(); loadCurrent(); }).catch(() => showLogin());

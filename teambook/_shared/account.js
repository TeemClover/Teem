import {
  TEAMBOOK_PROFILE_KEY, getProfile, handSizeOf, normalizeProfile, refreshParty, saveProfile,
} from './store.js';
import { TEAMBOOK_V1_PET_IDS } from './pets.js';
import { profilePortrait } from './card-picker.js';

const TEAMBOOK_TOKENS_KEY = 'teambook_book_tokens_v1';
const TEAMBOOK_PROFILE_IDS_KEY = 'teambook_profile_ids_v1';

/* Keep the first identity users see on / in lockstep with the editable
...
export function knownProfileIds() {
  const ids = [];
  try {
    const stored = JSON.parse(localStorage.getItem(TEAMBOOK_PROFILE_IDS_KEY) || '[]');
...
export function rememberProfileIds(...values) {
  ...
  try { localStorage.setItem(TEAMBOOK_PROFILE_IDS_KEY, JSON.stringify(merged)); } catch {}
  return merged;
}
...
export function startLineLogin(returnTo = '/') {
...
export async function bindXtyIdentity(profileId = getProfile()?.id) {
  ...
  return request('/api/teambook/bind', {
...
export async function loadCloudProgress() {
  ...
  const cloudProfile = parseProfile(result.profile);
  ...
  rememberProfileIds(parseIdList(result.profileIds), cloudProfile?.id ? [cloudProfile.id] : []);
  return { ...result, profile: cloudProfile };
}

export async function saveCloudProgress(profile = getProfile(), baseVersion = undefined) {
  ...
  return request('/api/profile', {
    ...
    body: JSON.stringify({
      profile: normalized,
      profileIds,
      ...(Number.isFinite(baseVersion) ? { baseVersion } : {}),
    }),
  });
}
...
function readRecoveryTokens() {
  try {
    const parsed = JSON.parse(localStorage.getItem(TEAMBOOK_TOKENS_KEY) || '{}');
...
function seedAccountPartyCodes(codes, meUserId) {
  ...
  try { localStorage.setItem(TEAMBOOK_TOKENS_KEY, JSON.stringify(map)); } catch {}
}

/* Restore active books from this TeamBook account only. Provider does not
   matter: LINE, Google and Email resolve to the same TeamBook user. Server
   state is canonical; localStorage is rebuilt only as a display/offline cache. */
export async function resyncXtyParties() {
  ...
  const mine = await request('/api/teambook-mine');
  ...
  /* Serialize local-cache writes. rememberResponse() does read/modify/write
     on teambook_books_v1, so parallel refreshes can race and make one restored
...
export async function syncXtyProfile({ recoverParties = true } = {}) {
  ...
  let saved = await saveCloudProgress(profile, Number(cloud.version || 0));
  if (saved.error === 'PROFILE_CONFLICT') {
    const retried = mergeXtyProfile(profile, saved.profile);
    if (retried) {
      const next = saveProfile(retried);
      refreshXtyHomeIdentity(next);
      saved = await saveCloudProgress(next, Number(saved.version || 0));
    }
  }
  let recovery = null;
...
/* Safari/iOS can restore / from BFCache without rerunning boot(). Repaint
...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('storage', event => {
    if (event.key === TEAMBOOK_PROFILE_KEY) refreshXtyHomeIdentity();
  });
}

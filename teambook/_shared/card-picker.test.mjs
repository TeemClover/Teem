...
const { mountCardPicker, avatarForChoice } = await import('./card-picker.js');
const { TEAMBOOK_CARDS } = await import('./cards.js');
const { TEAMBOOK_AVATARS } = await import('./avatars.js');

function setProfile(cardIds = []) {
  store.teambook_profile_v1 = JSON.stringify({
...
test('all five shelves are always present, empty ones locked at 0', () => {
  ...
  assert.equal(starter.owned, TEAMBOOK_AVATARS.length);
...
test('the picker never reveals how many cards exist', () => {
  ...
  const oneCard = TEAMBOOK_CARDS.find(c => c.rarity === 'epic');
...
test('owning one card unlocks exactly its own shelf', () => {
  const rare = TEAMBOOK_CARDS.find(c => c.rarity === 'rare');
...
test('the colour filter never empties Starter — those animals have no colour', () => {
  ...
  assert.equal(starter.owned, TEAMBOOK_AVATARS.length, 'starter survives a colour filter');
  assert.equal(starter.locked, false);
});

test('the colour filter narrows card shelves to that colour', () => {
  const reds = TEAMBOOK_CARDS.filter(c => c.rarity === 'common' && c.color === 'red');
  const blue = TEAMBOOK_CARDS.find(c => c.rarity === 'common' && c.color === 'blue');
...
test('a card you have not found is not shown at all', () => {
  const mine = TEAMBOOK_CARDS.find(c => c.rarity === 'common');
...
test('choosing a card carries the avatar to the same animal', () => {
  const card = TEAMBOOK_CARDS.find(c => c.rarity === 'rare');
...
test('selecting reports a choice the caller can act on', () => {
  const card = TEAMBOOK_CARDS.find(c => c.rarity === 'common');
...
test('the rarest card of an animal wins the portrait', async () => {
  const { bestCardForSpecies, petSpeciesFor } = await import('./card-picker.js');
  /* Only some animals have a rare printed, so pick one that does. */
  const rare = TEAMBOOK_CARDS.find(c => c.rarity === 'rare');
  const species = rare.species;
  const common = TEAMBOOK_CARDS.find(c => c.rarity === 'common' && c.species === species);
...
test('a card adds its animal to the Pet seat without removing the free ones', async () => {
  const { petSpeciesFor } = await import('./card-picker.js');
  const card = TEAMBOOK_CARDS.find(c => c.species === 'pig');
  setProfile([card.cardId]);
...
test('a card resolves to the plain animal for chat and the skin for seats', async () => {
  const { resolveMemberAvatar } = await import('./card-picker.js');
  const { avatarById } = await import('./avatars.js');
...
test('every printed card has its own art, and chat still shows the animal', async () => {
  ...
  for (const rarity of ['common', 'rare', 'epic', 'legendary']) {
    const card = TEAMBOOK_CARDS.find(c => c.rarity === rarity);
...
test('the stored avatar is the card when equipped, the animal otherwise', async () => {
  const { memberAvatarValue } = await import('./card-picker.js');
  const card = TEAMBOOK_CARDS.find(c => c.rarity === 'rare');

  setProfile([card.cardId]);
  const owned = JSON.parse(store.teambook_profile_v1);
  owned.equippedCardId = card.cardId;
  store.teambook_profile_v1 = JSON.stringify(owned);

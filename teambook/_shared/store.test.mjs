...
test('legacy profile migrates without inventing a starter card', () => {
  localStorage.setItem('teambook_profile_v1', JSON.stringify({
...
test('catalog matches the printed set with a minimal canonical card face', () => {
  ...
  const total = cards.TEAMBOOK_COMMON_CARDS.length + cards.TEAMBOOK_RARE_CARDS.length
  ...
  assert.equal(cards.TEAMBOOK_RARE_CARDS.length, 12);
  assert.equal(cards.TEAMBOOK_EPIC_CARDS.length, 12);
  assert.equal(cards.TEAMBOOK_LEGENDARY_CARDS.length, 8);
  ...
  assert.equal(cards.TEAMBOOK_CARDS.every(card => card.name === card.speciesNameTh), true);
  assert.equal(cards.TEAMBOOK_CARDS.every(card => card.accessoryColor === card.color), true);
  assert.equal(cards.TEAMBOOK_CARDS.every(card => [
    'image', 'flavorText', 'description', 'usableAsAvatar', 'usableAsNpc',
    'usableAsPartyCover', 'status', 'unlockMethod',
  ].every(key => key in card)), true);
  assert.equal(cards.TEAMBOOK_CARDS.every(card => !('playableInCore7' in card)), true);
...
test('activity metadata is one canonical source with legacy aliases and a safe custom fallback', () => {
  ...
  assert.equal(activities.activityMetadataById('trade').category, 'craft');
...
test('collection exhaustion returns complete instead of issuing a duplicate', () => {
  ...
  const printed = cards.TEAMBOOK_CARDS.length;
...
test('collection debug codes unlock and discard the whole data-driven catalog', () => {
  ...
  assert.equal(store.ownedCardIds().length, cards.TEAMBOOK_CARDS.length);
...
test('card placement only locks an owned role in a party controlled by this player', () => {
  ...
  localStorage.setItem('teambook_books_v1', JSON.stringify([party]));
  localStorage.setItem('teambook_book_tokens_v1', JSON.stringify({ '01234': { token: 'x', userId: 'u_member' } }));
  assert.equal(store.cardAvailability(reward.cardId).status, 'AVAILABLE');
  localStorage.setItem('teambook_books_v1', JSON.stringify([{ ...party, ownerId: 'u_owner' }]));
  localStorage.setItem('teambook_book_tokens_v1', JSON.stringify({ '01234': { token: 'x', userId: 'u_owner' } }));
...
test('an authenticated sync binds the local identity before merging', async () => {
  ...
  globalThis.fetch = async (path, options = {}) => {
    ...
    if (path === '/api/teambook/bind') return reply({ ok: true });
    /* cloud already holds a different profile id — the merge will adopt
       it, which is exactly why the bind has to happen first */
    if (path === '/api/profile' && (options.method || 'GET') === 'GET') {
      return reply({
        profile: { id: 'cloud-player', alias: 'คีน', updatedAt: '2026-08-02T00:00:00.000Z' },
        profileIds: ['cloud-player'],
        version: 1,
      });
    }
  ...
  const bindAt = paths.indexOf('/api/teambook/bind');
  ...
  assert.ok(bindAt < paths.lastIndexOf('/api/profile'), 'bind must run before the merge');
...
test('bind carries every profile id this device has ever used', async () => {
  localStorage.setItem('teambook_profile_ids_v1', JSON.stringify(['retired-one', 'retired-two']));
  localStorage.setItem('teambook_profile_v1', JSON.stringify({
  ...
  const realFetch = globalThis.fetch;
  globalThis.fetch = async (path, options = {}) => {
    if (path === '/api/teambook/bind') sent = JSON.parse(options.body);
...
test('party recovery keeps codes that are not five digits', async () => {
  ...
  globalThis.fetch = async (path, options = {}) => {
    ...
    if (path === '/api/teambook-mine') return reply({ ok: true, meUserId: 'account:acct1', codes: ['00042', '7', 'A19'] });
    if (String(path).startsWith('/api/teambook/party/')) {
...
test('an anonymous sync never binds', async () => {
  ...
  assert.equal(calls.includes('/api/teambook/bind'), false);
});

/* ═══════════════════════════════════════════════════════════════
   TeamBook — one card picker, used wherever a card becomes something
...
import { AVATAR_BY_ID, TEAMBOOK_AVATARS, avatarById, speciesById } from './avatars.js';
import { TEAMBOOK_CARDS, TEAMBOOK_CARD_COLORS, cardById, cardDescriptorTh, cardNameTh } from './cards.js';
...
function ownedCardsOf(rarity, owned) {
  return TEAMBOOK_CARDS.filter(card => (card.rarity || 'common') === rarity && owned.has(card.cardId));
}

function starterItems() {
  return TEAMBOOK_AVATARS.map(animal => ({
...
function installStyles() {
  ...
  style.textContent = `
    ...
    /* The selection control must never become a second card frame. TeamBook
    ...
    /* TeamBook collection cards fill the canonical 63×88 card face. */
    .xcp-opt>.animal-card{
...
export function mountCardPicker(host, options = {}) {
  ...
  let selectedKey = options.selected?.cardId ? `card:${options.selected.cardId}`
    : `starter:${options.selected?.avatarId || getProfile()?.avatarId || TEAMBOOK_AVATARS[0].id}`;
  ...
  function renderFilters() {
    ...
    const options = [{ id: '', label: 'ทุกสี' }].concat(
      TEAMBOOK_CARD_COLORS.map(id => ({ id, label: COLOR_LABEL[id] || id })),
    );
...
export function petSpeciesFor(grantedIds = [], profile = getProfile()) {
  ...
  const owned = ownedCardIds(profile);
  for (const card of TEAMBOOK_CARDS) if (owned.has(card.cardId)) fromCards.add(card.species);

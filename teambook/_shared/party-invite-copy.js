/* TeamBook party invite — deterministic clipboard copy.
   Product rule: Party sharing always stays inside TeamBook and gives the player
   ...
   This module runs only on /p/ and captures the legacy #copy button
...
function partySnapshot() {
  ...
  const name = String(party.name || document.getElementById('pname')?.textContent || 'TeamBook').trim() || 'TeamBook';
  ...
  const inviteUrl = `${location.origin}/join/?c=${encodeURIComponent(code)}`;

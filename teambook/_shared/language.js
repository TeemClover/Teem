/* TeamBook standard-language layer over the legacy TeamBook engine.
   Public UI speaks Book language; internal route/function/storage names may
   remain TeamBook while the product migrates. Human-authored names, intentions,
   rules, notes and canonical history are never rewritten. */

const STORAGE_KEY = 'teambook_language_mode';
const HAS_BROWSER = typeof window !== 'undefined' && typeof document !== 'undefined';
const ON_TEAMBOOK_SURFACE = HAS_BROWSER && (
  location.pathname === '/profile/' || location.pathname === '/profile' || location.pathname.startsWith('/')
);
...
const RULES = Object.freeze([
  ...
  [/DURABLE SAVE\s*·\s*TEAMBOOK ACCOUNT/gi, 'เก็บความคืบหน้ากับบัญชี TeamBook'],
  ...
  [/\bPROGRESS\b/gi, 'ความคืบหน้า'],
  [/\bMEMBERS\b/gi, 'คนในสมุด'],
  [/\bMEMBER\b/gi, 'คนในสมุด'],
...
function bootLanguage() {
  ...
  window.TeamBookLanguage = Object.freeze({
    getMode: () => 'plain',
    setMode: () => 'plain',

...
import {
  MESSAGE_MAX_CHARS, TEAMBOOK_TIMEZONE, completionGate, confirmDeadlineForDayKey,
...
function dayKey(date = new Date(), timezone = TEAMBOOK_TIMEZONE) { return partyDateKey(date, timezone); }
...
async function progressionFor(sql, ids, now = new Date()) {
  ...
  const rows = values.length
    ? await sql.query(`SELECT user_id,level,paid_tier,unlocked_bonus_slots FROM teambook_progression
...
async function membersOf(sql, partyId) {
  const rows = await sql.query(`SELECT user_id,alias,avatar,avatar_color,role,joined_at,left_at,removal_reason,
    activity_id,activity_label,activity_description,activity_color,success_rule FROM teambook_book_members
    WHERE book_id=$1 ORDER BY CASE role WHEN 'lead' THEN 0 ELSE 1 END, joined_at`, [partyId]);
  return rows.map(row => ({
...
async function issueCardRewardsForParty(sql, row, at = new Date()) {
  const members = await sql.query(`SELECT user_id FROM teambook_book_members
    WHERE book_id=$1 AND left_at IS NULL ORDER BY joined_at`, [row.id]);
  for (const membership of members) {
    const existing = await sql.query(`SELECT id,card_id,created_at FROM teambook_card_unlock_events
      WHERE user_id=$1 AND book_id=$2 AND unlock_source='ending' LIMIT 1`, [membership.user_id, row.id]);
    if (existing[0]) {
    ...
    const unlockEventId = `TEAMBOOK:BOOK:${row.id}:ENDING:USER:${membership.user_id}`;
    const inserted = await sql.query(`INSERT INTO teambook_card_unlock_events
      ...
      VALUES ($1,$2,$3,$4,$5,'ending','TeamBook',$6,$7)
      ON CONFLICT (unlock_event_id) DO NOTHING RETURNING id,card_id`, [
      rewardId, unlockEventId, membership.user_id, row.id, cardId, at, cardId ? null : at,
    ]);
...
async function postsOf(sql, partyId, partyCode, since = 0) {
  ...
  const reactions = await sql.query(`SELECT seq,user_id,emoji FROM teambook_reactions
    WHERE book_id=$1 AND seq>$2 ORDER BY seq`, [partyId, since]);
  ...
  const confirmations = await sql.query(`SELECT commit_seq,confirmer_id,created_at FROM teambook_confirmations
    WHERE book_id=$1 AND commit_seq>$2`, [partyId, since]);
...
async function stateFor(sql, row, member, since = 0) {
  ...
  const [memberHistory, posts, events, counts, progression, rewards] = await Promise.all([
    ...
    sql.query(`SELECT
      ...
      FROM teambook_book_entries WHERE book_id=$1`, [
  ...
  if (kind === 'message') {
    ...
    if (activeParty(row)) {
      rows = await sql.query(`WITH locked AS (
          SELECT id FROM teambook_books WHERE id=$1 AND state = ANY($7::text[]) FOR UPDATE
        ), allowed AS (
          SELECT id FROM locked WHERE (
            SELECT COUNT(*) FROM teambook_book_entries WHERE book_id=$1 AND user_id=$2
              AND kind='message' AND day_key=$3::date
          ) < $4
        ), next AS (
          UPDATE teambook_books p SET head_seq=p.head_seq+1,updated_at=$5
          FROM allowed a WHERE p.id=a.id RETURNING p.head_seq
        )
        INSERT INTO teambook_book_entries (book_id,seq,user_id,kind,body,sent_at,day_key,retracted,image_url,image_w,image_h)
      ...
      rows = await sql.query(`WITH locked AS (
          SELECT id,ended_at FROM teambook_books WHERE id=$1 AND state='COMPLETED' FOR UPDATE
        ), allowed AS (
          SELECT id FROM locked WHERE (
            SELECT COUNT(*) FROM teambook_book_entries WHERE book_id=$1 AND user_id=$2
...
async function intakeImage(partyCode, image) {
  ...
  try {
    ...
    console.error('TeamBook image upload failed', error);
...
async function applyProgressionForParty(sql, row, at = new Date()) {
  ...
  const members = await sql.query(`SELECT user_id,role,joined_at,left_at FROM teambook_book_members
    WHERE book_id=$1`, [row.id]);
  const partyStart = startOfPartyDay(row.started_at || row.created_at, row.timezone || TEAMBOOK_TIMEZONE);
  ...
  for (const membership of members) {
    ...
    if (progression.level === 1) reason = 'QUEST_COMPLETED';
      ...
      const joined = startOfPartyDay(membership.joined_at || partyStart, row.timezone || TEAMBOOK_TIMEZONE);
      const from = joined > partyStart ? joined : partyStart;
      const leftBoundary = membership.left_at
        ? startOfPartyDay(membership.left_at, row.timezone || TEAMBOOK_TIMEZONE)
        : partyEnd;
      ...
      const valid = await sql.query(`SELECT COUNT(DISTINCT p.day_key)::int n FROM teambook_book_entries p
        WHERE p.book_id=$1 AND p.user_id=$2 AND p.kind='commit' AND p.retracted=FALSE
          AND p.sent_at >= $3 AND p.sent_at < $4
          AND ($5='trust' OR EXISTS (SELECT 1 FROM teambook_confirmations c
    ...
    await sql.query(`WITH event AS (
        INSERT INTO teambook_level_events (user_id,book_id,from_level,to_level,reason,created_at)
        VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (user_id,book_id) DO NOTHING RETURNING user_id
      ) UPDATE teambook_progression SET level=$4,updated_at=$6
  ...
  try {
    ...
    if (method === 'POST' && parts[0] === 'bind') {
      ...
      await sql.query(`WITH local_members AS MATERIALIZED (
          SELECT DISTINCT ON (book_id) * FROM teambook_book_members WHERE user_id = ANY($1::text[])
          ORDER BY book_id,(left_at IS NULL) DESC,(role='lead') DESC,joined_at
        ), merged_members AS (
          INSERT INTO teambook_book_members (book_id,user_id,alias,avatar,avatar_color,role,auth_hash,joined_at,left_at,removal_reason)
          SELECT book_id,$2,alias,avatar,avatar_color,role,auth_hash,joined_at,left_at,removal_reason FROM local_members
          ...
          UPDATE teambook_reactions SET user_id=$2 WHERE user_id = ANY($1::text[])
            AND NOT EXISTS (SELECT 1 FROM teambook_reactions r2 WHERE r2.book_id=teambook_reactions.book_id
              AND r2.seq=teambook_reactions.seq AND r2.user_id=$2 AND r2.emoji=teambook_reactions.emoji)
          RETURNING book_id
        ), confirmations AS (
          ...
          UPDATE teambook_books SET owner_id=$2,updated_at=$3 WHERE owner_id = ANY($1::text[]) RETURNING id
        ), progression AS (
          INSERT INTO teambook_progression (user_id,level,paid_tier,unlocked_bonus_slots,updated_at)
          SELECT $2,level,paid_tier,unlocked_bonus_slots,$3 FROM teambook_progression
            WHERE user_id = ANY($1::text[]) ORDER BY level DESC,unlocked_bonus_slots DESC LIMIT 1
          ON CONFLICT (user_id) DO UPDATE SET level=GREATEST(teambook_progression.level,EXCLUDED.level),updated_at=$3
          RETURNING user_id
        ), deleted AS (
          DELETE FROM teambook_book_members WHERE user_id = ANY($1::text[]) AND EXISTS (SELECT 1 FROM merged_members) RETURNING book_id
        ) DELETE FROM teambook_progression WHERE user_id = ANY($1::text[])`, [localIds, accountId, at]);
      await sql.query(`WITH copied AS (
          INSERT INTO teambook_user_cards (user_id,card_id,acquired_from,acquired_at)
          SELECT DISTINCT ON (card_id) $2,card_id,acquired_from,acquired_at FROM teambook_user_cards
            WHERE user_id = ANY($1::text[]) ORDER BY card_id,acquired_at
          ON CONFLICT (user_id,card_id) DO NOTHING RETURNING card_id
        ) DELETE FROM teambook_user_cards WHERE user_id = ANY($1::text[])`, [localIds, accountId]);
    ...
    if (method === 'POST' && parts.length === 1) {
      ...
      const history = await sql.query(`SELECT COUNT(*)::int n FROM teambook_books
      ...
      const timezone = TEAMBOOK_TIMEZONE;
        ...
        try {
          const inserted = await sql.query(`WITH guard AS (
            ...
            SELECT 1 FROM guard WHERE
              (SELECT COUNT(DISTINCT p.id) FROM teambook_book_members m JOIN teambook_books p ON p.id=m.book_id
              ...
              AND ($23::text IS NULL OR NOT EXISTS (SELECT 1 FROM teambook_books
                WHERE owner_id IN ($13,$14) AND state = ANY($16::text[])
                  AND (lead_card_id=$23 OR npc_card_id=$23)))
              AND ($24::text IS NULL OR NOT EXISTS (SELECT 1 FROM teambook_books
                WHERE owner_id IN ($13,$14) AND state = ANY($16::text[])
                  AND (lead_card_id=$24 OR npc_card_id=$24)))
          ), party AS (
            INSERT INTO teambook_books (id,code,name,activity,commit_rule,budget,pet_id,owner_id,created_at,updated_at,
            ...
            SELECT id,$8,$10,$11,$31,'lead',$12,$9 FROM party RETURNING book_id
          ) INSERT INTO teambook_book_events (book_id,type,actor_id,party_day,data_json,created_at)
            SELECT book_id,'PARTY_CREATED',$8,1,$26::jsonb,$9 FROM lead_member RETURNING book_id`, [
    ...
    if (method === 'POST' && parts[2] === 'debug' && parts[3] === 'collection') {
      ...
      if (debugCode === 'discard') {
        await sql.query('DELETE FROM teambook_card_unlock_events WHERE user_id=$1', [member.user_id]);
        await sql.query('DELETE FROM teambook_user_cards WHERE user_id=$1', [member.user_id]);
    ...
    if (method === 'POST' && parts[2] === 'me') {
      ...
      if (aliasChanged || avatarChanged) {
        await sql.query(`WITH changed AS (
            ...
            SELECT book_id,$6,$5,$7,$8::jsonb,$9 FROM changed`, [
    ...
    if (method === 'POST' && parts[2] === 'finish') {
      ...
      const changed = await sql.query(`WITH party AS (
          UPDATE teambook_books SET state=$1,ended_at=$2,updated_at=$2,
          ...
          SELECT id,$5,$6,$7,'{}'::jsonb,$2 FROM party RETURNING book_id`, [
    ...
    if (method === 'POST' && parts[2] === 'manage') {
      ...
      if (action === 'rename') {
        ...
        if (name !== row.name) {
          ...
          await sql.query(`WITH party AS (
              UPDATE teambook_books SET name=$1,updated_at=$2 WHERE id=$3 RETURNING id
            ) INSERT INTO teambook_book_events (book_id,type,actor_id,party_day,data_json,created_at)
      ...
      if (action === 'change_rule') {
        ...
        if (rule !== old) {
          await sql.query(`WITH party AS (
              UPDATE teambook_books SET commit_rule=$1,updated_at=$2 WHERE id=$3 RETURNING id
            ) INSERT INTO teambook_book_events (book_id,type,actor_id,party_day,data_json,created_at)
      ...
      if (action === 'change_lead_card' || action === 'change_cover') {
        ...
        if (requestedType === 'image') {
          ...
          await sql.query(`WITH party AS (
              UPDATE teambook_books SET cover_type='image',cover_value=$1,lead_card_id=NULL,updated_at=$2
          ...
          if (previous && previous !== url) {
            const stillPosted = await sql.query(
              'SELECT 1 FROM teambook_book_entries WHERE book_id=$1 AND image_url=$2 LIMIT 1', [row.id, previous],
            );
        ...
        if (requestedType === 'card_back') {
          ...
          await sql.query(`WITH party AS (
              UPDATE teambook_books SET lead_card_id=NULL,cover_type='card_back',cover_value='notebook-rgbs-v1',updated_at=$1
        ...
        const occupied = await sql.query(`SELECT 1 FROM teambook_books WHERE owner_id=$1 AND code<>$2
        ...
        if (next !== old) {
          await sql.query(`WITH party AS (
              UPDATE teambook_books SET lead_card_id=$1,cover_type='card',cover_value=$1,updated_at=$2 WHERE id=$3 RETURNING id
            ) INSERT INTO teambook_book_events (book_id,type,actor_id,party_day,data_json,created_at)
              SELECT id,'LEAD_CARD_CHANGED',$4,$5,$6::jsonb,$2 FROM party`, [
      ...
      if (action === 'kick') {
        ...
        const rows = await sql.query(`SELECT user_id,alias,role FROM teambook_book_members
          WHERE book_id=$1 AND user_id=$2 AND left_at IS NULL`, [row.id, target]);
        const kicked = rows[0];
        ...
        await sql.query(`WITH member AS (
            UPDATE teambook_book_members SET left_at=$1,removal_reason='KICKED',auth_hash=NULL
            WHERE book_id=$2 AND user_id=$3 AND left_at IS NULL RETURNING book_id
          ), party AS (
            UPDATE teambook_books SET updated_at=$1 WHERE id=$2 AND EXISTS (SELECT 1 FROM member) RETURNING id
          ) INSERT INTO teambook_book_events (book_id,type,actor_id,party_day,data_json,created_at)
    ...
    if (method === 'POST' && parts[2] === 'react') {
      ...
      const post = await sql.query('SELECT 1 FROM teambook_book_entries WHERE book_id=$1 AND seq=$2', [row.id, seq]);
      if (!post[0]) return sendJson(res, { ok: false, error: 'NO_POST' }, 404);
      const at = new Date();
      const removed = await sql.query(`DELETE FROM teambook_reactions WHERE book_id=$1 AND seq=$2
        AND user_id=$3 AND emoji=$4 RETURNING emoji`, [row.id, seq, member.user_id, emoji]);
      if (!removed[0]) await sql.query(`INSERT INTO teambook_reactions (book_id,seq,user_id,emoji,created_at)
    ...
    if (method === 'POST' && parts[2] === 'confirm') {
      ...
      const commit = await sql.query(`SELECT user_id,day_key,sent_at FROM teambook_book_entries
        WHERE book_id=$1 AND seq=$2 AND kind='commit' AND retracted=FALSE`, [row.id, seq]);
      ...
      const commitDayKey = /^\d{4}-\d{2}-\d{2}$/.test(String(commit[0].day_key))
        ...
        : partyDateKey(commit[0].day_key, row.timezone || TEAMBOOK_TIMEZONE);
      const deadline = confirmDeadlineForDayKey(commitDayKey, row.timezone || TEAMBOOK_TIMEZONE);
      ...
      const saved = await sql.query(`INSERT INTO teambook_confirmations (book_id,commit_seq,confirmer_id,created_at)
        SELECT $1,p.seq,$2,$3 FROM teambook_book_entries p
        WHERE p.book_id=$1 AND p.seq=$4 AND p.kind='commit' AND p.retracted=FALSE AND p.user_id<>$2
        ON CONFLICT (book_id,commit_seq) DO NOTHING RETURNING commit_seq`, [row.id, member.user_id, at, seq]);
      if (!saved[0]) {
    ...
    if (method === 'POST' && parts[2] === 'retract') {
      const seq = Number(bodyOf(req).seq);
      if (!Number.isInteger(seq)) return sendJson(res, { ok: false, error: 'BAD_SEQ' }, 400);
      const saved = await sql.query(`UPDATE teambook_book_entries SET body='',retracted=TRUE WHERE book_id=$1
    ...
    console.error('TeamBook API failed', error);
    if (error.code === 'TEAMBOOK_DATABASE_URL_NOT_CONFIGURED') return sendJson(res, { ok: false, error: error.code }, 503);
    if (sql) {
      const errorCode = /^[A-Z0-9_]{2,60}$/i.test(String(error?.code || '')) ? String(error.code) : 'TEAMBOOK_API_ERROR';
      const endpoint = clean(new URL(req.url || '/', 'https://teambook.local').pathname, 160) || '/api/teambook';

...
async function memberFor(req, sql, partyId, account) {
  if (account?.id) {
    const rows = await sql.query(`SELECT user_id,role FROM teambook_book_members
...
export async function handleDebugLevel2(req, res) {
  try {
    ...
    const rows = await sql.query(`SELECT id,state FROM teambook_books WHERE code=$1`, [code]);
    const party = rows[0];
    ...
    const beforeRows = await sql.query(`SELECT level,paid_tier,unlocked_bonus_slots FROM teambook_progression
      WHERE user_id=$1 LIMIT 1`, [member.user_id]);
    const before = Number(beforeRows[0]?.level || 1);

    /* Deliberately do NOT insert teambook_level_events here. The active Quest must
    ...
    if (before < 2) {
      await sql.query(`UPDATE teambook_progression SET level=2,updated_at=$2
        WHERE user_id = ANY($1::text[]) AND level < 2`, [targets, at]);
    }

    const afterRows = await sql.query(`SELECT level,paid_tier,unlocked_bonus_slots FROM teambook_progression
      WHERE user_id=$1 LIMIT 1`, [member.user_id]);
    ...
    console.error('TeamBook level2 debug trigger failed', error);

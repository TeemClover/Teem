...
import {
  TEAMBOOK_TIMEZONE, normalizeVerificationMode, scheduledEndAt,
} from './xty-rules.js';
...
async function progressionFor(sql, ids) {
  ...
  const rows = await sql.query(`SELECT level,paid_tier,unlocked_bonus_slots FROM teambook_progression
...
function snapshot({ row, member, event, scheduled }) {
  return {
    ...
    state: 'ACTIVE',
    timezone: row.timezone || TEAMBOOK_TIMEZONE,
...
export async function handleCreatePartyV2(req, res) {
  try {
    ...
    const history = await sql.query(`SELECT COUNT(*)::int n FROM teambook_book_quota_v2
    ...
    for (let attempt = 0; attempt < 30; attempt += 1) {
      ...
      try {
        const eventData = JSON.stringify({
          ...
          guideId: petId === WHITE_CAT_GUIDE_ID ? WHITE_CAT_GUIDE_ID : null,
        });
        const rows = await sql.query(`WITH guard AS (
            SELECT pg_advisory_xact_lock(hashtext($1))
          ), usage AS (
            SELECT COUNT(*)::int n FROM teambook_book_quota_v2 q
            ...
            SELECT 1 FROM guard,usage WHERE usage.n < $3
              AND ($20::text IS NULL OR NOT EXISTS (
                SELECT 1 FROM teambook_book_quota_v2 q JOIN teambook_books p ON p.id=q.book_id
                WHERE q.quota_key=$1 AND q.role='owner' AND q.released_at IS NULL
            ...
            SELECT id,$11,$25,$26,$27,'lead',$28,$12,$13,$7,$31,$32,$33 FROM party RETURNING book_id
          ), quota AS (
            INSERT INTO teambook_book_quota_v2 (quota_key,book_id,role,created_at)
            SELECT $1,book_id,'owner',$12 FROM member RETURNING book_id
          )
          INSERT INTO teambook_book_events (book_id,type,actor_id,party_day,data_json,created_at)
          SELECT book_id,'PARTY_CREATED',$11,1,$29::jsonb,$12 FROM quota RETURNING book_id`, [
          ...
          TEAMBOOK_TIMEZONE, verificationMode, leadCardId, npcCardId, scheduled, coverType, coverValue,
        ...
        const partyRows = await sql.query(`SELECT id,code,name,activity,activity_id,preset,duration_days,color,visibility,
          ...
          verification_mode,scheduled_end_at,cover_type,cover_value,
          activity_mode,shared_activity_description,shared_activity_color
          FROM teambook_books WHERE id=$1`, [partyId]);
    ...
    console.error('TeamBook v2 party creation failed', error);

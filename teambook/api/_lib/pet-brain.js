/* ═══════════════════════════════════════════════════════════════
   TeamBook — living party pet brain
...
async function describeLinkedImages(history) {
  ...
  try {
    ...
    console.error('TeamBook pet vision prepass failed',
...
export async function readAndRespond({
  ...
  try {
    response = await groqRequest({
      ...
      response_format: {
        type: 'json_schema',
        json_schema: { name: 'teambook_pet_turn', strict: true, schema: DECISION_SCHEMA },
      },
    ...
    console.error('TeamBook pet brain Groq call failed', party.code, error?.status || error?.name || error);
    return null;
  }

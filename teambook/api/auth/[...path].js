...
function nativeSession(req, token) {
  return String(req.headers?.['x-teambook-client'] || '').toLowerCase() === 'native'
    ? { sessionToken: token }
    : {};
}
...
function accountReturn(returnTo, status, origin) {
  const back = new URL(safeReturn(returnTo || '/profile/'), origin);
...
export default async function handler(req, res) {
  try {
    ...
    if (method === 'POST' && route[0] === 'otp' && route[1] === 'request') {
      ...
      await sql.query(`UPDATE teambook_email_otps SET used_at=$1 WHERE normalized_email=$2 AND used_at IS NULL`, [createdAt, email]);
      await sql.query(`INSERT INTO teambook_email_otps
      ...
      if (!sent.ok) {
        await sql.query('DELETE FROM teambook_email_otps WHERE id=$1', [id]);
    ...
    if (method === 'POST' && route[0] === 'otp' && route[1] === 'verify') {
      ...
      const rows = await sql.query(`SELECT id,normalized_email,otp_hash,otp_salt,attempt_count,expires_at,used_at
        FROM teambook_email_otps WHERE id=$1`, [requestId]);
      ...
      if (!otpMatches(otp, row)) {
        await sql.query('UPDATE teambook_email_otps SET attempt_count=attempt_count+1 WHERE id=$1 AND used_at IS NULL', [requestId]);
        return sendJson(res, { ok: false, error: 'OTP_INVALID', message: 'รหัสไม่ถูกต้อง ลองอีกครั้งครับ' }, 401);
      }
      const claimed = await sql.query(`UPDATE teambook_email_otps SET used_at=$1
      ...
      let accounts = await sql.query('SELECT id,email,display_name,member_no FROM teambook_users WHERE email=$1', [email]);
      let account = accounts[0];
      if (!account) {
        const id = randomUUID(); const name = clean(body.name, 80) || 'สมาชิก';
        const memberNo = await ensureMemberNo(sql, email, name, now);
        accounts = await sql.query(`INSERT INTO teambook_users
      ...
      return sendJson(res, { ok: true, user: publicUser(account), ...nativeSession(req, session) }, 200, { 'Set-Cookie': sessionCookie(session) });
    }
    ...
    if (method === 'POST' && route[0] === 'signup') {
      ...
      const existing = await sql.query('SELECT id,password_hash FROM teambook_users WHERE email=$1', [email]);
      ...
      const memberNo = await ensureMemberNo(sql, email, name, now);
      await sql.query(`INSERT INTO teambook_users (id,email,display_name,password_hash,password_salt,password_iterations,member_no,consent_at,created_at,updated_at)
      ...
      await sql.query(`INSERT INTO teambook_auth_identities (provider,provider_user_id,user_id,email,created_at) VALUES ('email',$1,$2,$1,$3)`, [email, id, now]);
      const token = await createSession(sql, id); await prune(sql);
      return sendJson(res, { ok: true, user: { id, email, displayName: name, memberNo }, ...nativeSession(req, token) }, 201, { 'Set-Cookie': sessionCookie(token) });
    }
    ...
    if (method === 'POST' && route[0] === 'login') {
      ...
      const rows = validEmail(email) ? await sql.query('SELECT id,email,display_name,member_no,password_hash,password_salt,password_iterations FROM teambook_users WHERE email=$1', [email]) : [];
      ...
      return sendJson(res, { ok: true, user: publicUser(rows[0]), ...nativeSession(req, token) }, 200, { 'Set-Cookie': sessionCookie(token) });
    }
    ...
    if (method === 'GET' && route[0] === 'oauth' && route[2] === 'start') {
      const provider = route[1]; const returnTo = safeReturn(req.query.return || '/profile/'); const origin = requestOrigin(req);
    ...
    if (method === 'GET' && route[0] === 'oauth' && route[2] === 'callback') {
      ...
      if (!config || !state) return redirect(res, '/profile/?account=oauth_error');
    ...
    if (error.code === 'TEAMBOOK_DATABASE_URL_NOT_CONFIGURED') return sendJson(res, { ok: false, error: error.code, message: 'ระบบสมาชิกกำลังเชื่อมฐานข้อมูล กรุณาลองใหม่อีกครั้งครับ' }, 503);

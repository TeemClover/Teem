(()=>{
  ...
  if(head&&!document.getElementById('xircle-media-style')){
    ...
    style.textContent=`
      ...
      @media(max-width:620px){.x-media-wrap{margin:22px auto 30px}.x-media,.x-media-wrap:after{border-radius:20px}.x-media-replace>.x-media-wrap{width:100%}}
    `;
    head.appendChild(style);
  }

  const assets={
    hero:'/xircle/assets/campaign/hero-xircle.svg?v=2',
    habit:'/xircle/assets/campaign/habit-score.svg?v=2',
    hardware:'/xircle/assets/campaign/hardware.svg?v=2',
    body:'/xircle/assets/source/body-composition.svg?v=2',
    maxage:'/xircle/assets/source/maxage-canonical.svg?v=2',
    community:'/xircle/assets/campaign/community.svg?v=2',
    routine:'/xircle/assets/campaign/routinex.svg?v=2'
  };
  ...
  const media=(src,label,kind='campaign')=>{
    ...
    img.addEventListener('error',()=>{
      ...
      if(host&&host.classList.contains('x-media-replace')){
        host.classList.remove('x-media-replace');
      }
      wrap.remove();
    },{once:true});
  ...
  const replace=(host,src,label,kind='campaign')=>{
    ...
    host.appendChild(media(src,label,kind));
  };
  ...
  document.querySelectorAll('a[href^="#"]').forEach(a=>a.addEventListener('click',e=>{
    ...
    if(id&&id.length>1){const target=document.querySelector(id);if(target){e.preventDefault();target.scrollIntoView({behavior:'smooth'});}}
  }));
})();
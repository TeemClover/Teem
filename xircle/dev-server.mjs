...
<label>Page <select id="page"><option value="/xircle/">XIRCLE demo</option><option value="/xircle/#appointment">Appointment reveal</option><option value="/meet/?intent=health&from=xircle&open=booking">XIRCLE Meet handoff</option><option value="/meet/">Plain Meet</option><option value="/meet/?intent=health&from=other&open=booking">Unknown source</option><option value="/meet/?intent=health&intent=curious&from=xircle&open=booking">Duplicate intent</option></select></label>
<label>Fixture <select id="fixture"><option value="none">Normal</option><option value="legacy">Legacy completed day</option><option value="draft">Existing curious booking draft</option><option value="storage">Storage unavailable</option><option value="reduced">Reduced motion simulation</option><option value="noscript">JavaScript unavailable</option><option value="images">Scene images unavailable</option></select></label>
...
document.getElementById('inspect').onclick=()=>{const doc=frame.contentDocument,win=frame.contentWindow;const visible=el=>el.getClientRects().length&&win.getComputedStyle(el).visibility!=='hidden';const readStorage=key=>{try{return win.localStorage.getItem(key)}catch{return 'unavailable'}};const overflow=[...doc.querySelectorAll('body *')].filter(el=>visible(el)&&el.getBoundingClientRect().right>win.innerWidth+1&&!el.closest('svg')&&!el.closest('.scene-plates,.fragment-image,.flying-day,.shutter-flash')).map(el=>el.id||el.className||el.tagName);const images=[...doc.images].filter(img=>img.hasAttribute('src')).map(img=>({src:new URL(img.src).pathname,loaded:img.complete&&img.naturalWidth>0}));document.getElementById('checks').textContent=JSON.stringify({viewport:win.innerWidth,documentWidth:doc.documentElement.scrollWidth,scene:doc.querySelector('#stage')?.dataset.scene,phase:doc.querySelector('#stage')?.dataset.phase,overflow,images,loadedResources:win.performance.getEntriesByType('resource').map(r=>new URL(r.name).pathname),legacy:readStorage('xircle.local.v1'),draft:readStorage('myclover.meet.draft.v1')},null,2);};
</script></html>`;
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (extension === '.html') {
      ...
      if (fixture === 'images') html = html.replace(/\/xircle\/assets\/(v3|v5)\/[^\s"<>]+\.webp/g, '/xircle/assets/qa-missing.webp');

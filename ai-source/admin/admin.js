(() => {
  ...
  function drawDetail() {
    ...
    if(row.status==='admitted'){
      const access=node('section','section');access.append(node('h3','','เปิดสิทธิ์เรียนแล้ว'),node('p','','ผู้เรียนใช้บัญชี myClover อีเมล '+row.email+' เข้าห้องเรียนได้เลย ไม่ต้องกรอกรหัสโอนหรือรหัสเปิดคอร์ส'));
      const learner=state.stats?.learners?.find(x=>x.userId===row.accountId);
      access.append(node('p','',learner?.lastActivityAt?'มีบันทึกการเรียนแล้ว · ล่าสุด '+date(learner.lastActivityAt):'ยังไม่พบความคืบหน้าการเรียน ไม่ได้แปลว่าเข้าไม่ได้'));
      ...
      const link=node('a','primary','เปิดหน้าห้องเรียน /learn');link.href='/learn/?course=ai-sauce';link.target='_blank';link.rel='noopener';access.append(link,node('p','small','ลิงก์นี้เปิดด้วยบัญชีของผู้กด หากครูเปิดเองจะเป็นบัญชีครู ไม่ใช่การทดสอบแทนนักเรียน'));
      const copy=node('button','','คัดลอกข้อความแจ้งผู้เรียน');copy.type='button';copy.addEventListener('click',async()=>{try{await navigator.clipboard.writeText('เปิดสิทธิ์คอร์ส AI ใส่ซอสให้แล้วครับ เข้าเรียนที่ https://www.myclover.com/learn/?course=ai-sauce ด้วยบัญชีอีเมล '+row.email+' ได้เลย ไม่ต้องกรอกรหัสโอนหรือรหัสเปิดคอร์สครับ');notice('คัดลอกข้อความแล้ว ยังไม่ได้ส่งให้ผู้เรียน');}catch{notice('คัดลอกไม่สำเร็จ กรุณาคัดลอกลิงก์ห้องเรียนด้านบน','error');}});access.append(copy);panel.append(access);
    }

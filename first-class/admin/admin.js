(() => {
  ...
  const label = value => ({ submitted:'รอตรวจยอด', proof_requested:'ขอสลิป', paid:'ชำระแล้ว', pending:'รอสิทธิ์', granted:'First Class', ready:'พร้อมทำ', sent:'ส่งแล้ว', failed:'ผิดพลาด', needs_match:'จับคู่ Discord' }[value] || value || '—');
  ...
  function updateStats() {
    ...
    $('#statAttention').textContent = state.rows.filter(row => ['proof_requested'].includes(row.payment_status) || ['failed','needs_match'].includes(row.discord_role_status) || row.confirmation_email_status === 'failed').length;
  }
  function render() {
    ...
    list.innerHTML = rows.map(row => {
      ...
      return `<article class="person-card" data-id="${row.id}">
        <div class="identity"><strong>${esc(row.display_name)}</strong><span>@${esc(row.discord_username)}</span><small>${esc(row.email)}</small></div>
        <div class="meta"><b>โอน ${esc(String(row.transfer_time || '').slice(0,5))}</b><br>LINE: ${esc(row.line_id || '—')}<br>${new Date(row.created_at).toLocaleString('th-TH',{dateStyle:'short',timeStyle:'short'})}</div>
        <div class="tools">${tools.map(tool => `<span>${esc(tool)}</span>`).join('')}</div>
        <div class="status-stack"><div class="badges"><span class="badge ${esc(row.payment_status)}">${label(row.payment_status)}</span><span class="badge ${esc(row.discord_role_status)}">Discord: ${label(row.discord_role_status)}</span><span class="badge ${esc(row.confirmation_email_status)}">Email: ${label(row.confirmation_email_status)}</span></div>
          <div class="actions">${!paid ? `<button class="primary" data-action="confirm_payment">ยืนยัน 98 บาท</button><button data-action="request_proof">ขอสลิป</button>` : ''}${paid && !granted ? `<button class="purple" data-action="grant_role">มอบ Role แล้ว</button>` : ''}${row.emailDraft?.mailto ? `<a href="${esc(row.emailDraft.mailto)}" data-email="${row.id}">เปิดอีเมล</a>` : ''}${!row.attended ? `<button data-action="mark_attended">มาเรียนแล้ว</button>` : `<span class="badge paid">เข้าเรียนแล้ว</span>`}</div></div>
      </article>`;
    }).join('');
  ...
  $('#exportButton').addEventListener('click', () => { const keys=['reference','display_name','email','discord_username','ai_tools','transfer_time','line_id','payment_status','first_class_status','discord_role_status','confirmation_email_status','attended','created_at']; const cell=value=>`"${String(Array.isArray(value)?value.join(' | '):value??'').replaceAll('"','""')}"`; const csv='\uFEFF'+[keys.join(','),...state.rows.map(row=>keys.map(key=>cell(row[key])).join(','))].join('\n'); const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));link.download='first-class-registrations.csv';link.click();URL.revokeObjectURL(link.href); });

...
- `PageView` + `ViewContent`: browser, production hosts only.
- `Lead`: browser, after the registration API confirms the record was saved.
...
The Pixel ID is intentionally exposed through the public config endpoint. The CAPI token is never returned to the browser.

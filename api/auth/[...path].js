...
async function sendOtpEmail(email, otp) {
  ...
  const from = process.env.MYCLOVER_FROM_EMAIL || process.env.XTY_FROM_EMAIL || process.env.FIRST_CLASS_FROM_EMAIL;
  if (!key || !from) return { ok: false, error: 'EMAIL_DELIVERY_NOT_CONFIGURED' };
  const response = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { authorization: `Bearer ${key}`, 'content-type': 'application/json' },
    body: JSON.stringify({
      from, to: [email], subject: 'รหัสเข้าสู่บัญชี myClover',
      text: `รหัส myClover ของคุณคือ ${otp}\n\nใช้ยืนยันอีเมลและเข้าสู่บัญชีเดิมของคุณ ทั้งคอร์สและความคืบหน้า\nรหัสนี้ใช้ได้ 10 นาที และใช้ได้ครั้งเดียว\nถ้าคุณไม่ได้ขอรหัสนี้ ไม่ต้องทำอะไร`,
    }),
  });
  return response.ok ? { ok: true } : { ok: false, error: 'EMAIL_DELIVERY_FAILED' };
}

export function googleVerifiedForAccount(identity, account) {
  return identity?.provider === 'google' && validEmail(identity.email || '')
    && identity.email.trim().toLowerCase() === String(account?.email || '').trim().toLowerCase();
}

function validOtpBody(req) {
...
async function googleIdentity(code, verifier, redirectUri, config) {
  ...
  if (!profileResponse.ok || !profile.sub || profile.email_verified !== true) throw new Error('google_profile');
...
return async function handler(req, res) {
  ...
  try {
    ...
    if (method === 'GET' && route[0] === 'providers') return sendJson(res, { ok: true, providers: {
      email: true,
      otp: Boolean(process.env.RESEND_API_KEY && (process.env.MYCLOVER_FROM_EMAIL || process.env.XTY_FROM_EMAIL || process.env.FIRST_CLASS_FROM_EMAIL)),
      google: !!providerConfig('google'), line: !!providerConfig('line'),
    } });
    ...
    if (method === 'POST' && route[0] === 'otp' && route[1] === 'request') {
      ...
      if (!validOtpBody(req)) return sendJson(res, { ok: false, error: 'INVALID_BODY' }, 400);
      ...
      if (await authRateLimited(sql, req, 'email-otp-request-ip', '', 20, 60)
        || await authRateLimited(sql, req, 'email-otp-request', email, 5, 60)) {
        return sendJson(res, { ok: false, error: 'RATE_LIMITED', message: 'ขอรหัสถี่เกินไป ลองใหม่ภายหลังครับ' }, 429);
      }
      const otp = String(randomInt(1000000)).padStart(6, '0');
      ...
      let sent;
      try { sent = await deliverOtp(email, otp); } catch { sent = { ok: false, error: 'EMAIL_DELIVERY_FAILED' }; }
      if (!sent.ok) {
    ...
    if (method === 'POST' && route[0] === 'otp' && route[1] === 'verify') {
      if (!sameOrigin(req)) return sendJson(res, { ok: false, error: 'BAD_ORIGIN' }, 403);
      if (!validOtpBody(req)) return sendJson(res, { ok: false, error: 'INVALID_BODY' }, 400);
      const body = bodyOf(req); const requestId = body.requestId; const otp = body.otp;
      if (typeof requestId !== 'string' || !/^[a-f0-9-]{36}$/i.test(requestId)
        || typeof otp !== 'string' || !/^\d{6}$/.test(otp)) return sendJson(res, { ok: false, error: 'OTP_INVALID' }, 400);
      const now = new Date();
      // Reserve one of the five guesses atomically before hashing. Concurrent
      // requests cannot all read the same attempt count and bypass the limit.
      const rows = await sql.query(`UPDATE mc_email_otps SET attempt_count=attempt_count+1
        WHERE id=$1 AND used_at IS NULL AND expires_at>$2 AND attempt_count<$3
        RETURNING id,normalized_email,otp_hash,otp_salt,attempt_count,expires_at,used_at`, [requestId, now, OTP_MAX_ATTEMPTS]);
      const row = rows[0];
      if (!row) return sendJson(res, { ok: false, error: 'OTP_EXPIRED', message: 'รหัสหมดอายุ ใช้แล้ว หรือครบจำนวนลอง ขอรหัสใหม่ได้เลยครับ' }, 410);
      ...
      if (!account) {
        ...
        accounts = await sql.query(`INSERT INTO mc_accounts
          ...
          VALUES ($1,$2,$3,NULL,NULL,NULL,$4,$5,$5,$5)
          ON CONFLICT(email) DO UPDATE SET email=EXCLUDED.email RETURNING id,email,display_name,member_no`,
      ...
      await sql.query('UPDATE mc_accounts SET email_verified_at=COALESCE(email_verified_at,$2) WHERE id=$1', [account.id,now]);
      account.email_verified_at = now;
      const session = await createSession(sql, account.id, { emailVerified: true }); await prune(sql);
    ...
    if (method === 'GET' && route[0] === 'oauth' && route[2] === 'callback') {
      ...
      try {
        ...
        const account = await accountForIdentity(sql, identity);
        const emailVerified = googleVerifiedForAccount(identity, account);
        if (emailVerified) await sql.query('UPDATE mc_accounts SET email_verified_at=COALESCE(email_verified_at,$2) WHERE id=$1', [account.id,new Date()]);
        const token = await createSession(sql, account.id, { emailVerified });
      ...
      } catch { console.error('OAuth callback failed', provider); return redirect(res, accountReturn(state.return_to, 'oauth_error', requestOrigin(req))); }
    }

...
import {
  XTY_TIMEZONE, partyDateKey, validPartyCode,
...
async function memberFor(req, sql, partyId) {
  ...
  if (account) {
    const rows = await sql.query(`SELECT user_id FROM xty_members
...
async function intakeImage(partyCode, image) {
  ...
  try {
    ...
    console.error('XTY commit image upload failed', error);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rows = await sql.query(`SELECT id,state,timezone,activity_mode,activity_id,activity,
      shared_activity_color FROM xty_parties WHERE code=$1`, [partyCode]);
    const row = rows[0];
    ...
    const now = new Date();
    const key = partyDateKey(now, row.timezone || XTY_TIMEZONE);
    const text = clean(body.note, 300) || '✓';

    try {
      await sql.query(`WITH next AS (
          UPDATE xty_parties SET head_seq=head_seq+1,updated_at=$4
    ...
    /* Reuse the canonical GET shaper after the write, so commit responses stay
       byte-for-byte compatible with the rest of the existing XTY client. */
    const proxy = Object.create(req);
    proxy.method = 'GET';
    proxy.url = `/api/xty/party/${encodeURIComponent(partyCode)}`;
    proxy.query = { ...(req.query || {}), path: ['party', partyCode] };
    return xtyHandler(proxy, res);
  } catch (error) {
    console.error('XTY commit route failed', error);
    if (sql) {
      await sql.query(`INSERT INTO xty_system_errors (error_code,endpoint,created_at)
        VALUES ($1,$2,$3)`, [String(error?.code || 'XTY_COMMIT_ERROR').slice(0, 60), '/api/xty/party/[code]/commit', new Date()]).catch(() => {});
    }

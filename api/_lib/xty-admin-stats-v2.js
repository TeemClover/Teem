...
import { XTY_TIMEZONE, partyDayNumber, startOfPartyDay } from './xty-rules.js';
...
async function safeQuery(sql, label, statement, values, warnings) {
  try {
    ...
    console.error(`XTY admin stats query failed: ${label}`, error);
...
export async function getAdminSummary(sql, rangeValue, at = new Date()) {
  ...
  const [users, activeUsers, parties, activity, cards, quest, confirms, progression, otp, durations] = await Promise.all([
    ...
    safeQuery(sql, 'summary.parties', `SELECT
      ...
      FROM xty_parties`, partyParams, warnings),
    ...
    safeQuery(sql, 'summary.activity', `SELECT
        ...
        (SELECT COUNT(*) FROM xty_reactions WHERE created_at IS NOT NULL AND ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2)::int reactions,
    ...
    safeQuery(sql, 'summary.cards', `SELECT
      ...
      FROM xty_card_rewards`, params, warnings),

    safeQuery(sql, 'summary.quest', `WITH cohort AS (
        SELECT * FROM xty_parties WHERE ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2
        ...
        COUNT(*) FILTER (WHERE state='COMPLETED' AND EXISTS (
          SELECT 1 FROM xty_card_rewards r WHERE r.party_id=cohort.id AND r.revealed_at IS NOT NULL
        ))::int reward_revealed
    ...
    safeQuery(sql, 'summary.progression', `WITH users AS (
        ...
        SELECT owner_id user_id,COUNT(*)::int n FROM xty_parties WHERE state=ANY($1::text[]) GROUP BY owner_id
      ), shaped AS (
    ...
    safeQuery(sql, 'summary.otp', `SELECT
      ...
      FROM mc_email_otps`, params, warnings),
    ...
    safeQuery(sql, 'summary.durations', `SELECT duration_days,
        ...
        COUNT(*) FILTER (WHERE state='COMPLETED' AND ($1::timestamptz IS NULL OR ended_at >= $1) AND ended_at <= $2)::int completed,
        COUNT(*) FILTER (WHERE state='DISSOLVED' AND ($1::timestamptz IS NULL OR ended_at >= $1) AND ended_at <= $2)::int dissolved
      FROM xty_parties WHERE duration_days IN (7,14,28) GROUP BY duration_days ORDER BY duration_days`, params, warnings),
  ]);
...
export async function getAdminActivity(sql, rangeValue, at = new Date()) {
  ...
  const [posts, reactions, confirms, partyEvents] = await Promise.all([
    safeQuery(sql, 'activity.posts', `SELECT (sent_at AT TIME ZONE 'Asia/Bangkok')::date day,
      ...
      FROM xty_posts WHERE ($1::timestamptz IS NULL OR sent_at >= $1) AND sent_at <= $2
    ...
    safeQuery(sql, 'activity.reactions', `SELECT (created_at AT TIME ZONE 'Asia/Bangkok')::date day,COUNT(*)::int reactions
      FROM xty_reactions WHERE created_at IS NOT NULL AND ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2
      GROUP BY 1 ORDER BY 1`, values, warnings),
    safeQuery(sql, 'activity.confirms', `SELECT (created_at AT TIME ZONE 'Asia/Bangkok')::date day,COUNT(*)::int confirms
      FROM xty_confirmations WHERE ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2
      GROUP BY 1 ORDER BY 1`, values, warnings),
    safeQuery(sql, 'activity.partyEvents', `SELECT (created_at AT TIME ZONE 'Asia/Bangkok')::date day,
      ...
      FROM xty_party_events WHERE ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2
      GROUP BY 1 ORDER BY 1`, values, warnings),
  ]);
...
export async function getAdminParties(sql, rawQuery = {}, at = new Date()) {
  ...
  const warnings = [];
  const values = [startOfPartyDay(at, XTY_TIMEZONE)];
  const where = [];
  ...
  if (query.occupancy === 'full') where.push(`(SELECT COUNT(*) FROM xty_members m WHERE m.party_id=p.id AND m.left_at IS NULL)>=5`);
  if (query.occupancy === 'joinable') {
    where.push(`p.state=ANY(${activeParam()}::text[])`);
    where.push(`(SELECT COUNT(*) FROM xty_members m WHERE m.party_id=p.id AND m.left_at IS NULL)<5`);
  }
  ...
  const statement = `SELECT p.id,p.code,p.name,p.activity,p.activity_id,p.state,p.visibility,p.verification_mode,
      ...
      (SELECT COUNT(*) FROM xty_members m WHERE m.party_id=p.id AND m.left_at IS NULL)::int member_count,
      (SELECT m.alias FROM xty_members m WHERE m.party_id=p.id AND m.role='lead' AND m.left_at IS NULL ORDER BY m.joined_at LIMIT 1) lead_alias,
      (SELECT m.avatar FROM xty_members m WHERE m.party_id=p.id AND m.role='lead' AND m.left_at IS NULL ORDER BY m.joined_at LIMIT 1) lead_avatar,
      (SELECT COUNT(*) FROM xty_posts x WHERE x.party_id=p.id AND x.kind='commit' AND x.retracted=FALSE AND x.day_key=$1::date)::int commits_today,
      (SELECT COUNT(*) FROM xty_posts x WHERE x.party_id=p.id AND x.kind='message' AND x.retracted=FALSE AND x.day_key=$1::date)::int messages_today,
      (SELECT COUNT(*) FROM xty_reactions r WHERE r.party_id=p.id AND r.created_at >= $1)::int reactions_today,
      (SELECT COUNT(*) FROM xty_posts q LEFT JOIN xty_confirmations c ON c.party_id=q.party_id AND c.commit_seq=q.seq
        WHERE q.party_id=p.id AND q.kind='commit' AND q.retracted=FALSE AND c.commit_seq IS NULL
  ...
  try {
    ...
    console.error('XTY admin party query failed; falling back to base party rows', error);
    ...
    rows = await safeQuery(sql, 'parties.fallback', `SELECT id,code,name,activity,activity_id,state,visibility,verification_mode,
      ...
      FROM xty_parties ORDER BY updated_at DESC,id DESC LIMIT $1 OFFSET $2`, fallbackValues, warnings);
  }
  ...
  const page = rows.slice(0, query.limit).map(row => ({
    ...
    day: Math.min(n(row.duration_days) || 7, partyDayNumber(row.started_at || row.created_at, at, row.timezone || XTY_TIMEZONE)),

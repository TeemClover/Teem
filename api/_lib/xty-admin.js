...
export async function handleXtyAdmin(req, res, sql, method, parts) {
  try {
    ...
    if (method === 'POST' && (action === 'party-dissolve' || action === 'ghosts')) {
      ...
      /* A party nobody can reach: still active, but not one live member is
         bound to a myClover account, so no login can ever recover or end it. */
      if (action === 'ghosts') {
        const rows = await sql.query(`SELECT p.code,p.name,p.state,p.created_at,p.updated_at,
          ...
          WHERE p.state = ANY($1::text[])
            AND NOT EXISTS (
              ...
              WHERE a.party_id=p.id AND a.left_at IS NULL AND a.user_id LIKE 'account:%'
            )

...
import { notifyBooking, deliveryStatus, bookingFromRow } from './meet-notify.js';
import { randomUUID, timingSafeEqual } from 'node:crypto';

const INTENTS = new Set(['health', 'opportunity', 'curious', 'ai']);
const AI_TOPICS = Object.freeze({
  private: 'เรียน 1–1 / Executive coaching',
  team: 'เวิร์กช็อปทีม / องค์กร',
  course: 'คอร์สย่อยของทีม',
  academy: 'แนะนำคอร์ส Pi R Academy',
  explore: 'ช่วยเลือกวิธีเรียน AI',
});
...
function authorized(req, config) {
  const wanted = config.MEET_ADMIN_KEY || '';
...
async function deliver(sql, booking, adminUrl, notify) {
  const result = await notify(booking, adminUrl);
...
/** The production handler and local preview share every validation and query.
 * Only I/O boundaries are injected; local tests never need production credentials. */
export function createMeetHandler({ database, sendJson, notify = notifyBooking,
  config = process.env, makeReference = reference, receipt = null }) {
  return async function handler(req, res) {
    ...
    try {
      ...
      if (req.method === 'POST') {
        ...
        if (intent === 'ai') {
          const topic = data.topic == null || data.topic === '' ? 'explore' : data.topic;
          if (typeof topic !== 'string' || !Object.hasOwn(AI_TOPICS, topic)) return sendJson(res, { ok: false, field: 'topic', message: 'รูปแบบการเรียน AI ไม่ถูกต้อง' }, 400);
          if (mode !== 'ออนไลน์' && mode !== 'เจอกันจริง') return sendJson(res, { ok: false, field: 'mode', message: 'เลือกรูปแบบการคุยเรื่อง AI อีกครั้ง' }, 400);
          // Existing queue, notification retries and exports already retain note.
          // Persist only a server-owned bounded topic label, never the incoming URL.
          const prefix = `[AI · ${AI_TOPICS[topic]}]`;
          note = note ? `${prefix} ${note.slice(0, 999 - prefix.length)}` : prefix;
        }
        if (intent === 'health' && data.focus != null && data.focus !== '') {
          if (typeof data.focus !== 'string' || !Object.hasOwn(HEALTH_FOCUS, data.focus)) return sendJson(res, { ok: false, field: 'focus', message: 'จุดเริ่มที่อยากคุยไม่ถูกต้อง' }, 400);
          const prefix = `[XIRCLE · จุดเริ่ม: ${HEALTH_FOCUS[data.focus]}]`;
          note = note ? `${prefix} ${note.slice(0, 999 - prefix.length)}` : prefix;
        }
        if (intent === 'opportunity' && data.entry === 'compass') {
          const context = [];
          for (const [field, registry] of [['need', OPPORTUNITY_NEEDS], ['offer', OPPORTUNITY_OFFERS]]) {
            const value = data[field];
            if (value == null || value === '') continue;
            if (typeof value !== 'string' || !Object.hasOwn(registry, value)) return sendJson(res, { ok: false, field, message: 'เรื่องที่อยากคุยต่อไม่ถูกต้อง' }, 400);
            context.push(registry[value]);
          }
          // Bounded discussion context only: not a promised match, job or income.
          const prefix = `[Compass · โอกาสและคนร่วมทาง${context.length ? ` · ${context.join(' · ')}` : ''}]`;
          note = note ? `${prefix} ${note.slice(0, 999 - prefix.length)}` : prefix;
        }
        ...
        const rows = await sql.query(
          ...
          [makeReference(), intent, mode, day, time, name, contact, note || null,
        ...
        const delivery = await deliver(sql, booking, origin ? `${origin}/meet/admin/` : '', notify);
        return sendJson(res, { ok: true, reference: booking.reference, notified: delivery.status === 'delivered', ...(receipt ? { receipt } : {}) }, 201);
      }

      const access = authorized(req, config);
      if (!access.ok) {
      ...
      if (req.method === 'GET') {
        ...
        return sendJson(res, {
          ok: true,
          channels: {
            line: Boolean(config.LINE_CHANNEL_ACCESS_TOKEN && config.LINE_NOTIFY_USER_ID),
      ...
      if (req.method === 'PATCH') {
        ...
        if (action === 'retry_notify') {
          ...
          const origin = requestOrigin(req);
          const delivery = await deliver(sql, bookingFromRow(row), origin ? `${origin}/meet/admin/` : '', notify);
          return sendJson(res, { ok: true, notify: delivery });
        }

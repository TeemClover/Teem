import { XTY_CARDS, XTY_CARD_COLORS, XTY_CARD_RARITIES, cardById } from '../../xty/_shared/cards.js';
import { XTY_TIMEZONE, partyDayNumber, scheduledEndAt, startOfPartyDay } from './xty-rules.js';
...
export function normalizeAdminRange(value, at = new Date()) {
  ...
  const today = startOfPartyDay(now, XTY_TIMEZONE);
...
async function integrityReport(sql) {
  const [badRewards, duplicateRewards, duplicateOwnership, placements, ownedIds] = await Promise.all([
    sql.query(`SELECT r.id,r.card_id,p.code,p.state FROM xty_card_rewards r
      LEFT JOIN xty_parties p ON p.id=r.party_id
      WHERE p.id IS NULL OR p.state<>'COMPLETED' ORDER BY r.created_at DESC LIMIT 100`),
    sql.query(`SELECT user_id,party_id,COUNT(*)::int n FROM xty_card_rewards
      GROUP BY user_id,party_id HAVING COUNT(*)>1 LIMIT 100`),
    sql.query(`SELECT user_id,card_id,COUNT(*)::int n FROM xty_card_ownership
      GROUP BY user_id,card_id HAVING COUNT(*)>1 LIMIT 100`),
    sql.query(`SELECT owner_id,card_id,COUNT(*)::int n FROM (
        SELECT owner_id,lead_card_id card_id FROM xty_parties WHERE state=ANY($1::text[]) AND lead_card_id IS NOT NULL
        UNION ALL SELECT owner_id,npc_card_id FROM xty_parties WHERE state=ANY($1::text[]) AND npc_card_id IS NOT NULL
      ) locked GROUP BY owner_id,card_id HAVING COUNT(*)>1 LIMIT 100`, [ACTIVE_STATES]),
    sql.query(`SELECT card_id,'ownership' source FROM xty_card_ownership
      UNION SELECT card_id,'reward' source FROM xty_card_rewards WHERE card_id IS NOT NULL`),
  ]);
  const issues = [];
  badRewards.forEach(row => issues.push({ type: 'REWARD_WITHOUT_COMPLETED_QUEST', party: row.code || 'missing', cardId: row.card_id || null }));
  duplicateRewards.forEach(row => issues.push({ type: 'MULTIPLE_REWARDS_SAME_QUEST_USER', partyId: row.party_id, count: count(row.n) }));
  ...
  const commonCovers = await sql.query(`SELECT code,lead_card_id FROM xty_parties
...
export async function getAdminSummary(sql, rangeValue, at = new Date()) {
  ...
  const [users, activeUsers, parties, activity, cards, quest, confirmHealth, progression, otp, collection, durations, integrity] = await Promise.all([
    sql.query(`WITH seen AS (
        SELECT user_id,MIN(joined_at) first_seen FROM xty_members GROUP BY user_id
        UNION ALL SELECT user_id,MIN(acquired_at) FROM xty_card_ownership GROUP BY user_id
        UNION ALL SELECT user_id,MIN(created_at) FROM xty_card_rewards GROUP BY user_id
        UNION ALL SELECT user_id,MIN(updated_at) FROM xty_progression GROUP BY user_id
      ), users AS (SELECT user_id,MIN(first_seen) first_seen FROM seen GROUP BY user_id), meaningful AS (
        SELECT user_id FROM xty_progression WHERE level>1
        UNION SELECT user_id FROM xty_card_ownership
        UNION SELECT m.user_id FROM xty_members m JOIN xty_parties p ON p.id=m.party_id WHERE p.state='COMPLETED'
        UNION SELECT m.user_id FROM xty_members m WHERE EXISTS (
          SELECT 1 FROM xty_members peer WHERE peer.party_id=m.party_id AND peer.user_id<>m.user_id
        )
    ...
    sql.query(`WITH activity AS (
        SELECT user_id,sent_at at FROM xty_posts WHERE retracted=FALSE
        UNION ALL SELECT user_id,created_at FROM xty_reactions WHERE created_at IS NOT NULL
        UNION ALL SELECT confirmer_id,created_at FROM xty_confirmations
        UNION ALL SELECT actor_id,created_at FROM xty_party_events WHERE actor_id IS NOT NULL
        UNION ALL SELECT user_id,joined_at FROM xty_members
        UNION ALL SELECT user_id,revealed_at FROM xty_card_rewards WHERE revealed_at IS NOT NULL
    ...
    sql.query(`SELECT
      ...
      FROM xty_parties`, partyParams),
    sql.query(`SELECT
        (SELECT COUNT(*) FROM xty_posts WHERE kind='commit' AND retracted=FALSE AND ($1::timestamptz IS NULL OR sent_at >= $1) AND sent_at <= $2)::int commits,
        (SELECT COUNT(*) FROM xty_posts WHERE kind='message' AND retracted=FALSE AND ($1::timestamptz IS NULL OR sent_at >= $1) AND sent_at <= $2)::int messages,
        (SELECT COUNT(*) FROM xty_reactions WHERE ($1::timestamptz IS NULL OR created_at >= $1) AND (created_at IS NULL OR created_at <= $2))::int reactions,
        (SELECT COUNT(*) FROM xty_confirmations WHERE ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2)::int confirms,
        (SELECT COUNT(*) FROM xty_reactions WHERE created_at IS NULL)::int legacy_reactions_without_timestamp,
        (SELECT COUNT(*) FROM xty_posts p JOIN xty_parties q ON q.id=p.party_id
    ...
    sql.query(`SELECT
        ...
        (SELECT COUNT(DISTINCT user_id) FROM xty_card_ownership)::int owners
      FROM xty_card_rewards`, rangeParams),
    sql.query(`WITH cohort AS (SELECT * FROM xty_parties
        WHERE ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2)
      SELECT COUNT(*)::int created,
        COUNT(*) FILTER (WHERE (SELECT COUNT(*) FROM xty_members m WHERE m.party_id=cohort.id)>=2)::int second_member,
        COUNT(*) FILTER (WHERE EXISTS (SELECT 1 FROM xty_posts p WHERE p.party_id=cohort.id AND p.kind='commit' AND p.retracted=FALSE))::int first_commit,
        ...
        COUNT(*) FILTER (WHERE state='COMPLETED' AND EXISTS (SELECT 1 FROM xty_card_rewards r WHERE r.party_id=cohort.id AND r.revealed_at IS NOT NULL))::int reward_revealed,
        COUNT(*) FILTER (WHERE state='COMPLETED' AND EXISTS (
          SELECT 1 FROM xty_members finished JOIN xty_members later ON later.user_id=finished.user_id
          JOIN xty_parties next_party ON next_party.id=later.party_id
          WHERE finished.party_id=cohort.id AND next_party.id<>cohort.id AND next_party.created_at>cohort.ended_at
        ))::int next_quest
    ...
    sql.query(`SELECT
      ...
      FROM xty_posts p JOIN xty_parties q ON q.id=p.party_id
      LEFT JOIN xty_confirmations c ON c.party_id=p.party_id AND c.commit_seq=p.seq
    ...
    sql.query(`WITH users AS (
        SELECT user_id FROM xty_members UNION SELECT user_id FROM xty_progression
        UNION SELECT user_id FROM xty_card_ownership UNION SELECT user_id FROM xty_card_rewards
      ), active_owned AS (SELECT owner_id user_id,COUNT(*)::int n FROM xty_parties WHERE state=ANY($1::text[]) GROUP BY owner_id),
      shaped AS (SELECT u.user_id,COALESCE(p.level,1)::int level,COALESCE(p.paid_tier,'free') tier,
        ...
        FROM users u LEFT JOIN xty_progression p ON p.user_id=u.user_id LEFT JOIN active_owned a ON a.user_id=u.user_id)
    ...
    sql.query(`SELECT COUNT(*) FILTER (WHERE ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2)::int sent,
      ...
      FROM mc_email_otps`, rangeParams),
    sql.query(`WITH users AS (
        SELECT user_id FROM xty_members UNION SELECT user_id FROM xty_progression
        UNION SELECT user_id FROM xty_card_ownership UNION SELECT user_id FROM xty_card_rewards
      ), owned AS (SELECT user_id,COUNT(*)::int n FROM xty_card_ownership GROUP BY user_id), shaped AS (
    ...
    sql.query(`SELECT duration_days,
        ...
        COUNT(*) FILTER (WHERE state='COMPLETED' AND ($1::timestamptz IS NULL OR ended_at >= $1) AND ended_at <= $2)::int completed,
        COUNT(*) FILTER (WHERE state='DISSOLVED' AND ($1::timestamptz IS NULL OR ended_at >= $1) AND ended_at <= $2)::int dissolved
      FROM xty_parties WHERE duration_days IN (7,14,28) GROUP BY duration_days ORDER BY duration_days`, rangeParams),
    integrityReport(sql),
  ]);
...
export async function getAdminParties(sql, rawQuery = {}, at = new Date()) {
  ...
  const where = []; const values = [ACTIVE_STATES, startOfPartyDay(at, XTY_TIMEZONE)];
  ...
  const rows = await sql.query(`SELECT p.id,p.code,p.name,p.activity,p.activity_id,p.state,p.visibility,p.verification_mode,
    ...
    LEFT JOIN LATERAL (SELECT COUNT(*) FILTER (WHERE left_at IS NULL) member_count,
      ...
      FROM xty_members WHERE party_id=p.id) ms ON TRUE
    ...
    LEFT JOIN LATERAL (SELECT COUNT(*) FILTER (WHERE c.commit_seq IS NULL AND q.day_key >= (($2::timestamptz AT TIME ZONE 'Asia/Bangkok')::date - 1)) pending_confirms,
      MAX(c.created_at) last_confirm FROM xty_posts q LEFT JOIN xty_confirmations c
      ON c.party_id=q.party_id AND c.commit_seq=q.seq WHERE q.party_id=p.id AND q.kind='commit' AND q.retracted=FALSE) cs ON TRUE
  ...
  const page = rows.slice(0, query.limit).map(row => ({
    ...
    day: Math.min(count(row.duration_days) || 7, partyDayNumber(row.started_at || row.created_at, at, row.timezone || XTY_TIMEZONE)),
...
export async function getAdminPartyDetail(sql, code, at = new Date()) {
  const parties = await sql.query(`SELECT id,code,name,activity,activity_id,commit_rule,state,visibility,verification_mode,
    ...
    FROM xty_parties WHERE code=$1 LIMIT 1`, [String(code || '')]);
  ...
  const [members, progress, activity, reward, events] = await Promise.all([
    sql.query(`SELECT user_id,alias,avatar,avatar_color,role,joined_at,left_at,removal_reason
      FROM xty_members WHERE party_id=$1 ORDER BY joined_at`, [party.id]),
    sql.query(`SELECT p.day_key,COUNT(*) FILTER (WHERE p.kind='commit' AND p.retracted=FALSE)::int submitted,
      ...
      FROM xty_posts p JOIN xty_parties q ON q.id=p.party_id
      LEFT JOIN xty_confirmations c ON c.party_id=p.party_id AND c.commit_seq=p.seq
      WHERE p.party_id=$1 GROUP BY p.day_key ORDER BY p.day_key`, [party.id]),
    sql.query(`SELECT
      ...
      (SELECT COUNT(*) FROM xty_reactions WHERE party_id=$1)::int reactions,
      (SELECT COUNT(*) FROM xty_confirmations WHERE party_id=$1)::int confirms
      FROM xty_posts WHERE party_id=$1`, [party.id]),
    sql.query(`SELECT COUNT(*)::int issued,COUNT(*) FILTER (WHERE revealed_at IS NOT NULL)::int revealed,
      COUNT(*) FILTER (WHERE revealed_at IS NULL)::int pending FROM xty_card_rewards WHERE party_id=$1`, [party.id]),
    sql.query(`SELECT type,party_day,data_json,created_at FROM xty_party_events
      WHERE party_id=$1 ORDER BY created_at DESC LIMIT 200`, [party.id]),
  ]);
  ...
  return {
    party: {
      ...
      day: Math.min(durationDays, partyDayNumber(party.started_at || party.created_at, at, party.timezone || XTY_TIMEZONE)),
...
export async function getAdminUsers(sql, rawQuery = {}, at = new Date()) {
  ...
  if (query.active) {
    ...
    const threshold = new Date(startOfPartyDay(at, XTY_TIMEZONE).getTime() - (days - 1) * 86400000);
  ...
  const rows = await sql.query(`WITH users AS (
      SELECT user_id FROM xty_members UNION SELECT user_id FROM xty_progression
      UNION SELECT user_id FROM xty_card_ownership UNION SELECT user_id FROM xty_card_rewards
    ), latest AS (SELECT DISTINCT ON (user_id) user_id,alias,avatar,avatar_color FROM xty_members ORDER BY user_id,joined_at DESC),
    first_seen AS (SELECT user_id,MIN(at) created_at FROM (
      SELECT user_id,joined_at at FROM xty_members UNION ALL SELECT user_id,acquired_at FROM xty_card_ownership
      UNION ALL SELECT user_id,created_at FROM xty_card_rewards UNION ALL SELECT user_id,updated_at FROM xty_progression
    ) x GROUP BY user_id),
    member_stats AS (SELECT m.user_id,COUNT(DISTINCT p.id) FILTER (WHERE p.owner_id=m.user_id)::int created_parties,
      ...
      FROM xty_members m JOIN xty_parties p ON p.id=m.party_id GROUP BY m.user_id),
    card_stats AS (SELECT user_id,COUNT(*)::int cards FROM xty_card_ownership GROUP BY user_id),
    ...
    FROM users u LEFT JOIN latest l ON l.user_id=u.user_id LEFT JOIN xty_progression pr ON pr.user_id=u.user_id
...
export async function getAdminCards(sql) {
  const [rewardRows, ownedRows, usageRows, integrity] = await Promise.all([
    sql.query(`SELECT card_id,COUNT(*)::int issued,COUNT(*) FILTER (WHERE revealed_at IS NOT NULL)::int revealed,
      COUNT(*) FILTER (WHERE revealed_at IS NULL)::int pending FROM xty_card_rewards GROUP BY card_id`),
    sql.query('SELECT card_id,COUNT(DISTINCT user_id)::int owned FROM xty_card_ownership GROUP BY card_id'),
    sql.query(`SELECT card_id,COUNT(*)::int in_use FROM (
      SELECT lead_card_id card_id FROM xty_parties WHERE state=ANY($1::text[]) AND lead_card_id IS NOT NULL
      UNION ALL SELECT npc_card_id FROM xty_parties WHERE state=ANY($1::text[]) AND npc_card_id IS NOT NULL
  ...
  const catalog = XTY_CARDS.map(card => ({
  ...
  return {
    summary: {
      ...
      usersOwningCards: count((await sql.query('SELECT COUNT(DISTINCT user_id)::int n FROM xty_card_ownership'))[0]?.n),
      integrityIssues: integrity.issues,
    },
    rarity: completeCardGroups(aggregateCards(catalog, 'rarity'), XTY_CARD_RARITIES),
    color: completeCardGroups(aggregateCards(catalog, 'color'), XTY_CARD_COLORS),
...
export async function getAdminActivity(sql, rangeValue, at = new Date()) {
  ...
  const rows = await sql.query(`WITH events AS (
      SELECT (sent_at AT TIME ZONE 'Asia/Bangkok')::date day,
        ...
        0::int parties_created,0::int parties_completed FROM xty_posts
        WHERE ($1::timestamptz IS NULL OR sent_at >= $1) AND sent_at <= $2 GROUP BY day
      UNION ALL SELECT (created_at AT TIME ZONE 'Asia/Bangkok')::date,0,0,COUNT(*)::int,0,0,0 FROM xty_reactions
        WHERE created_at IS NOT NULL AND ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2 GROUP BY 1
      UNION ALL SELECT (created_at AT TIME ZONE 'Asia/Bangkok')::date,0,0,0,COUNT(*)::int,0,0 FROM xty_confirmations
      ...
      UNION ALL SELECT (created_at AT TIME ZONE 'Asia/Bangkok')::date,0,0,0,0,
        ...
        FROM xty_party_events WHERE ($1::timestamptz IS NULL OR created_at >= $1) AND created_at <= $2 GROUP BY 1
...
export async function getAdminSystem(sql) {
  const [integrity, errors, codes, logins] = await Promise.all([
    ...
    sql.query(`SELECT error_code,endpoint,COUNT(*)::int count,MAX(created_at) last_seen
      FROM xty_system_errors GROUP BY error_code,endpoint ORDER BY last_seen DESC LIMIT 100`),
    sql.query(`SELECT COUNT(*)::int live_codes,COUNT(*)-COUNT(DISTINCT code)::int duplicate_codes
      FROM xty_parties WHERE state=ANY($1::text[])`, [ACTIVE_STATES]),
    sql.query(`SELECT type,COUNT(*)::int count,MAX(created_at) last_seen FROM xty_admin_audit
...
export async function getAdminEvents(sql, rawQuery = {}) {
  ...
  const party = queryText(rawQuery.party_id).slice(0, 80);
  ...
  const rows = await sql.query(`WITH all_events AS (
      SELECT created_at,type,party_id,actor_id,data_json FROM xty_party_events
      UNION ALL SELECT created_at,'CARD_REWARDED',party_id,user_id,jsonb_build_object('cardId',card_id) FROM xty_card_rewards
      UNION ALL SELECT revealed_at,'CARD_REVEALED',party_id,user_id,jsonb_build_object('cardId',card_id)
        FROM xty_card_rewards WHERE revealed_at IS NOT NULL
      UNION ALL SELECT created_at,'LEVEL_UP',party_id,user_id,jsonb_build_object('from',from_level,'to',to_level,'reason',reason)
        FROM xty_level_events
      UNION ALL SELECT created_at,'EMAIL_BOUND',NULL,'account:'||user_id,'{}'::jsonb FROM mc_auth_identities WHERE email IS NOT NULL
      UNION ALL SELECT created_at,type,NULL,NULL,data_json FROM xty_admin_audit
    ) SELECT e.created_at,e.type,e.actor_id,e.data_json,p.code,p.name FROM all_events e
      LEFT JOIN xty_parties p ON p.id=e.party_id
      WHERE ($1='' OR e.type=$1) AND ($2='' OR e.party_id=$2 OR p.code=$2)

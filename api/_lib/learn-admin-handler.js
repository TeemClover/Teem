...
import { getLearnCourse } from './learn-catalog.js';
...
export function progressForLearner(course,rows) {
  ...
  const lessons=course.lessons.map(lesson=>{
    const row=byId.get(lesson.id),cap=Number(lesson.durationSeconds)||86400;
    const bounded=value=>Math.min(cap,Math.max(0,Number(value)||0));
    return {id:lesson.id,title:lesson.title,positionSeconds:bounded(row?.position_seconds),
...
export function createLearnAdminHandler({getSql=database,config=process.env,storeFactory=createLearnAdminStore,now=()=>new Date(),courseLookup=getLearnCourse}={}) {
  return async(req,res)=>{
    ...
    try {
      ...
      for(const k of ['courseId','limit','before'])if(query.getAll(k).length>1)return reply({ok:false,code:'INVALID_QUERY'},400);
      const course=courseLookup(query.get('courseId')||'ai-sauce');if(!course)return reply({ok:false,code:'COURSE_NOT_FOUND'},404);
      const rawLimit=query.get('limit')||'50',before=query.get('before')||null;
      if(!/^\d{1,3}$/.test(rawLimit)||Number(rawLimit)<1||Number(rawLimit)>100||(before&&!/^[a-zA-Z0-9_-]{1,128}$/.test(before)))return reply({ok:false,code:'INVALID_QUERY'},400);
      ...
      return reply({ok:true,course:{id:course.id,title:course.title,totalLessons:course.lessons.length},generatedAt:time.toISOString(),
        ...
        learners,nextCursor:rawLearners.length>limit?page.at(-1).user_id:null,

...
export async function handleXtyBind(req, res) {
  ...
  try {
    ...
    /*
      ...
      The previous binder could move xty_parties.owner_id to the account while
    ...
    */
    await sql.query(`WITH local_members AS MATERIALIZED (
        ...
        UPDATE xty_parties SET owner_id=$2,updated_at=$3
        ...
        WHERE m.user_id = ANY($1::text[])
          AND EXISTS (SELECT 1 FROM local_members lm WHERE lm.party_id=m.party_id)
        RETURNING party_id
      )
      SELECT
    ...
    /* Repair parties that were already affected by an older bad merge. If the
       ...
       database/manual fix. */
    await sql.query(`INSERT INTO xty_members
        (party_id,user_id,alias,avatar,avatar_color,role,auth_hash,joined_at,left_at,removal_reason)
      SELECT p.id,$1,
        COALESCE((SELECT m.alias FROM xty_members m WHERE m.party_id=p.id ORDER BY (m.role='lead') DESC,m.joined_at LIMIT 1),$2),
      ...
      [accountId, String(account.displayName || account.email || 'Clover').slice(0, 24)]);

...
async function ensureQuotaV2(sql) {
  await sql.query(`CREATE TABLE IF NOT EXISTS xty_party_quota_v2 (
    quota_key TEXT NOT NULL, party_id TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'owner',
...
async function partyByCode(sql, code) {
  const rows = await sql.query(`SELECT id,code,name,state,owner_id,created_at,started_at,updated_at,timezone,
    lead_card_id,npc_card_id,cover_type,cover_value FROM xty_parties WHERE code=$1`, [code]);
  return rows[0] || null;
}
async function memberFor(req, sql, partyId) {
  ...
  if (account) {
    const rows = await sql.query(`SELECT user_id,alias,avatar,avatar_color,role,joined_at
      FROM xty_members WHERE party_id=$1 AND user_id=$2 AND left_at IS NULL`, [partyId, `account:${account.id}`]);
    if (rows[0]) return { account, member: rows[0] };
  }
  const token = bearer(req);
  if (!token) return { account, member: null };
  const rows = await sql.query(`SELECT user_id,alias,avatar,avatar_color,role,joined_at
    FROM xty_members WHERE party_id=$1 AND auth_hash=$2 AND left_at IS NULL`, [partyId, await sha256(token)]);
...
function coverInfo(row) {
  ...
  if (type === 'core7_card' && row?.cover_value) {
    const card = core7CardById(row.cover_value);
    return { type, value: row.cover_value, name: card ? core7Name(card) : row.cover_value };
  }
  if (type === 'avatar') return { type, value: row?.cover_value || '', name: 'Animal Avatar · Legacy Cover' };
  return { type: 'card_back', value: 'myclover-back-v1', name: 'หลังการ์ด myClover' };
}
...
async function stateViaLegacy(legacyXtyHandler, req, code) {
  ...
  proxy.method = 'GET'; proxy.url = `/api/xty/party/${encodeURIComponent(code)}`;
...
export async function handleProfileV2(req, res) {
  try {
    ...
    const targetRows = await sql.query(`SELECT user_id,alias,avatar,avatar_color,role,joined_at FROM xty_members
      WHERE party_id=$1 AND user_id=$2 AND left_at IS NULL LIMIT 1`, [row.id, targetId]);
    ...
    const [activeRows, progressionRows] = await Promise.all([
      ...
      sql.query(`SELECT level,unlocked_bonus_slots FROM xty_progression WHERE user_id=$1 LIMIT 1`, [targetId]),
    ]);
    ...
    console.error('XTY profile v2 failed', error);
...
export async function handleCoverV2(req, res, legacyXtyHandler) {
  try {
    ...
    let nextType = 'card_back'; let nextValue = 'myclover-back-v1'; let nextLead = null; let nextName = 'หลังการ์ด myClover';
    if (requested === 'card') {
      ...
      const owned = await sql.query(`SELECT 1 FROM xty_card_ownership WHERE user_id=$1 AND card_id=$2 LIMIT 1`, [member.user_id, id]);
      if (!owned[0]) return sendJson(res, { ok: false, error: 'CARD_NOT_OWNED' }, 403);
      const occupied = await sql.query(`SELECT 1 FROM xty_parties WHERE id<>$1 AND owner_id=$2
    ...
    } else if (requested === 'core7_card') {
      const id = clean(body.core7CardId || body.coverValue, 80).toLowerCase(); const card = core7CardById(id);
      if (!card || card.generic || !String(card.id || '').startsWith('fh-')) return sendJson(res, { ok: false, error: 'CARD_NOT_COVER_ELIGIBLE' }, 400);
      const occupied = await sql.query(`SELECT 1 FROM xty_parties WHERE id<>$1 AND owner_id=$2
        AND state = ANY($3::text[]) AND cover_type='core7_card' AND cover_value=$4 LIMIT 1`, [row.id, member.user_id, ACTIVE_STATES, id]);
      if (occupied[0]) return sendJson(res, { ok: false, error: 'CARD_IN_USE' }, 409);
      nextType = 'core7_card'; nextValue = id; nextName = core7Name(card);
    }
    ...
    await sql.query(`WITH changed AS (
        UPDATE xty_parties SET cover_type=$1,cover_value=$2,lead_card_id=$3,updated_at=$4 WHERE id=$5 RETURNING id
    ...
    console.error('XTY cover v2 failed', error);
    if (error.code === 'DATABASE_URL_NOT_CONFIGURED') return sendJson(res, { ok: false, error: error.code }, 503);
...
export async function handleIdentityV2(req, res, legacyXtyHandler) {
  try {
    ...
    await sql.query(`WITH changed AS (UPDATE xty_members SET alias=$1,avatar=$2,avatar_color=$3
      ...
      touched AS (UPDATE xty_parties SET updated_at=$6 WHERE id=$4 AND EXISTS (SELECT 1 FROM changed) RETURNING id)
      INSERT INTO xty_party_events (party_id,type,actor_id,party_day,data_json,created_at)

...
const page = readFileSync('xty/start/index.html', 'utf8');
const css = readFileSync('xty/_shared/start.css', 'utf8');

/* /xty/start/ is the front door: the one URL that has to work for a stranger
...
test('a returning visitor is sent to their shelf before the pitch paints', () => {
  ...
  assert.match(head, /mc_xty_profile/, 'the skip must read the profile key');
  assert.match(head, /location\.replace\('\/xty\/'\)/, 'and go straight to the shelf');
  /* Anything that runs after the body has started is too late — the visitor
     would see a flash of a page that is not for them. */
  assert.ok(head.indexOf('mc_xty_profile') < page.indexOf('<body'),
...
test('a brief names its size and its destination path', () => {
  ...
  for (const brief of briefs) {
    ...
    assert.match(brief, /xty\/assets\/start\/[a-z-]+\.webp<\/code>/, 'and where the file goes');
  }
});
...
test('both ways in are offered, and neither needs an account', () => {
  assert.match(page, /href="\/xty\/new\/"/, 'open a book');
  assert.match(page, /href="\/xty\/join\/"/, 'or join one with a code');

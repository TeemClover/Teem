/* XTY ต้องยืนได้บนโดเมนไหนก็ได้

   teambook.me จะเสิร์ฟ XTY โดยที่ myclover.com ยังเสิร์ฟส่วนอื่นอยู่ แปลว่า
   หน้า XTY ทุกหน้าต้องไม่มีลิงก์ที่ลากผู้เล่นข้ามโดเมนกลับบ้านเดิมกลางเกม

   ตอนเขียนเทสต์นี้ XTY สะอาดอยู่แล้ว — ไม่มีลิงก์คลิกได้ที่ชี้ออกนอกโดเมนเลย
...
const XTY = join(dirname(fileURLToPath(import.meta.url)), '..');
...
test('no XTY page links a player back off the domain they are playing on', () => {
  const offenders = [];
  for (const path of pages(XTY)) {
    for (const url of clickableLinks(readFileSync(path, 'utf8'))) {
      if (/myclover\.com|teambook\.me/i.test(url)) {
        offenders.push(`${path.slice(XTY.length + 1)} → ${url}`);
      }
    }
  }
  assert.deepEqual(offenders, [],
    'ใช้ path แบบ /xty/... แทน URL เต็ม ไม่งั้นผู้เล่นบน teambook.me จะถูกเด้งกลับ myclover.com');
});

test('no XTY canonical points at a host that only redirects', () => {
  const offenders = [];
  for (const path of pages(XTY)) {
    ...
    if (canonical && /xty\.myclover\.com/i.test(canonical[1])) {
      offenders.push(`${path.slice(XTY.length + 1)} → ${canonical[1]}`);
    }
  }
  assert.deepEqual(offenders, []);
});

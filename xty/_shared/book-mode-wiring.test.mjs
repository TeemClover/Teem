...
const route = readFileSync('api/xty/[...path].js', 'utf8');
const newPage = readFileSync('xty/new/index.html', 'utf8');
const joinPage = readFileSync('xty/join/index.html', 'utf8');
...
test('the columns the feature needs all exist', () => {
  for (const column of ['activity_mode', 'shared_activity_description', 'shared_activity_color']) {
    assert.match(core, new RegExp(`xty_parties ADD COLUMN IF NOT EXISTS ${column}`), `parties.${column}`);
  }
  for (const column of ['activity_id', 'activity_label', 'activity_description', 'activity_color', 'success_rule']) {
    assert.match(core, new RegExp(`xty_members ADD COLUMN IF NOT EXISTS ${column}`), `members.${column}`);
  }
  for (const column of ['activity_id', 'activity_label', 'activity_color', 'success_rule_snapshot']) {
    assert.match(core, new RegExp(`xty_posts ADD COLUMN IF NOT EXISTS ${column}`), `posts.${column}`);
  }
});
...
test('a signature copies the activity rather than pointing at it', () => {
  ...
  const sql = route.match(/INSERT INTO xty_posts \([\s\S]*?'commit'[\s\S]*?RETURNING seq/)?.[0];
  ...
  assert.match(sql, /JOIN xty_members m ON m\.party_id=\$1 AND m\.user_id=\$2/);
...
test('a stranger is told which kind of book this is before they join', () => {
  ...
  assert.match(joinPage, /fetch\(`\/api\/xty\/party\/\$\{encodeURIComponent\(code\)\}`/,
...
test('nothing left behind still refers to the old flat picker', () => {
  for (const dead of ['customActivity', 'activity-choice', 'XTY_ACTIVITIES']) {

...
test('2.0 maps the complete browser module graph to one release in game and preview', async () => {
  ...
  assert.match(html, /game\.css\?v=2\.0-opportunity1/);
  assert.match(html, /game-boot\.js\?v=2\.0-opportunity1/);
  ...
  for (const name of [...canonicalModules, 'game-boot.js']) {
    ...
    assert.equal(map[canonical], `${canonical}?v=2.0-opportunity1`, `${name} must bypass earlier release caches`);
    assert.equal(previewMap[canonical], map[canonical], `preview must use the same ${name}`);
  }
  assert.equal(previewMap['/tests/xvisor/game-review-fixtures.mjs'], '/tests/xvisor/game-review-fixtures.mjs?v=2.0-opportunity1');

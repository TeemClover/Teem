...
import { makeInitialState, serializeState, SAVE_KEY, EVENTS, STAGES } from '../../xvisor/quest/game-data.js';
...
async function load(page,state) {
  ...
  await page.waitForFunction(() => performance.getEntriesByType('resource').some(r => r.name.includes('neighborhood-scenes-v2.png') && r.responseEnd>0));
}
...
try {
  for(const width of [1440,390,320]) {
    ...
    try {
      ...
      assert.match(await page.locator('#goalTitle').textContent(),/Live/);
      ...
      legacy.prospects = [{...legacy.prospects[0],journey:'recommendation',routinePlan:{id:'all',quality:'poor',products:['gus','protein-hmb','vita-matrix','astamega']}}];
      ...
      check(`${width}px: the old คุยแฟ้ม X button advances a saved full plan once without reopening RoutineX choices`);

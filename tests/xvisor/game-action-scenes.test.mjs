...
import { createSceneAtlas } from '../../xvisor/quest/game-scene-atlas.js';
import { MENTOR_PALETTES } from '../../xvisor/quest/game-art.js';
...
test('action locations vary deterministically and retain the actual cover guide identities', () => {
  const previous = management();
  ...
  assert.equal(MENTOR_PALETTES.teem.characterId, 'teem');
  assert.equal(MENTOR_PALETTES.teem.glasses, true);
  assert.equal(MENTOR_PALETTES.ako.hairStyle, 'ponytail');
  assert.equal(MENTOR_PALETTES.ako.dress, true);
});
...
test('scene atlas has immediate fallback, exact quadrant crops, load invalidation and cleanup', t => {
  const original = Object.getOwnPropertyDescriptor(globalThis, 'Image');
  const instances = [];
  class FakeImage {
    complete = false; naturalWidth = 0; naturalHeight = 0; listeners = new Map();
  ...
  let invalidations = 0;
  const atlas = createSceneAtlas(() => invalidations++);
  const calls = [];
  const context = { save() {}, restore() {}, drawImage(...args) { calls.push(args.slice(1)); } };
  assert.equal(atlas.draw(context, 'kitchen'), false);
  assert.match(instances[0].src, /neighborhood-scenes-v2\.png$/);
  const image = instances[0];
  Object.assign(image, { complete: true, naturalWidth: 1672, naturalHeight: 941 });
  image.listeners.get('load')();
  assert.equal(invalidations, 1);
  assert.equal(atlas.draw(context, 'kitchen'), true);
  assert.deepEqual(calls.at(-1), [837, 1, 834, 468.5, 0, 0, 384, 216]);
  assert.equal(atlas.draw(context, 'studio'), true);
  assert.deepEqual(calls.at(-1), [1, 471.5, 834, 468.5, 0, 0, 384, 216]);
  atlas.destroy();
  assert.equal(image.listeners.size, 0);
  assert.equal(atlas.draw(context, 'office'), false);
});
...
function renderEnvironment(t, reduced = false) {
  ...
  let sequence = 0;
  const calls = [];
  const brush = new Proxy({}, { get(target, property) {

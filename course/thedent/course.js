...
(() => {
  ...
  const resourceNames = {
    ...
    'ai-leader-homework.md':'การบ้าน AI Leader · 7 วัน', 'advanced-course.md':'Advance · ซอสกลางบน GitHub',
  ...
  function startIntro(){return `<div class="start-intro"><div class="intro-label">เริ่มที่นี่ · ฟังก่อนลงมือ</div><h2>เปิดสไลด์ AI ใส่ซอส</h2><p>30 นาทีแรกใช้สไลด์ 20 หน้าปูพื้นฐาน ฟังและคุยกันก่อน แล้วเริ่มเปิดเครื่องมือเวลา 14:30</p><div class="button-row"><a class="button primary" href="./opening.html#slide/1">เปิดสไลด์ปูพื้นฐาน ↗</a><a class="button" href="#learn/sauce">14:30 · เริ่มลงมือ →</a><button type="button" class="text-button" data-resource="opening-notes.md">โน้ตผู้สอน</button></div></div><div class="outcomes-grid"><article class="outcome"><div class="outcome-number">01</div><div><div class="outcome-tag">CONTENT + ADMIN</div><h3>ซอสเดียว สื่อสารได้ทั้งทีม</h3><p>หลังปูพื้นฐาน ทำโพสต์และคำตอบแอดมินจากข้อมูลจริง</p><a href="#learn/workshop1">ดู Workshop 1 ↗</a></div></article><article class="outcome"><div class="outcome-number">02</div><div><div class="outcome-tag">EXCEL + CSV</div><h3>ไฟล์จากระบบ สู่สรุปพร้อมใช้</h3><p>จัดข้อมูล · ทำสรุป · ใช้ซ้ำกับไฟล์รอบหน้า</p><a href="#learn/workshop2">ดู Workshop 2 ↗</a></div></article></div>`;}
  ...
  function advanceLesson() {
    return `<section class="advance-next" aria-labelledby="advance-heading"><p class="eyebrow">NEXT COURSE · 1 วัน</p><h2 id="advance-heading">จากเก่งคนเดียว สู่ซอสกลางขององค์กร</h2><p>AI Leader Advance · นำ Source ขึ้น GitHub และให้ทีมใช้ข้อมูลเวอร์ชันเดียวกัน</p><ol class="advance-flow"><li>ติดตั้งและเตรียมเครื่อง</li><li>จัดซอสและสิทธิ์ของทีม</li><li>ฝึกอัปเดตและใช้ร่วมกัน</li></ol><div class="button-row"><a class="button primary" href="./advance.html">ดูคอร์ส Advance และสิ่งที่จะสร้าง ↗</a><span>คอร์สถัดไป · มีค่าเรียนแยก</span></div></section><p class="followup-included">นัดติดตามกลุ่ม 30 นาทีในอีก 1–2 สัปดาห์ รวมอยู่ในคอร์สนี้แล้ว มาเปิดผลงานและแก้จุดติดด้วยกัน</p>`;
  }
  ...
  function showResource(file){
    ...
    showDialog(resourceNames[file],`<p class="dialog-intro">${esc(file)}</p><div class="button-row">${file==='advanced-course.md'?'<a class="button" href="./advance.html">เปิดหน้าคอร์ส Advance ↗</a>':''}<a class="button primary" href="./resources/${esc(file)}" download>ดาวน์โหลดไฟล์ ↓</a>${resource!==undefined?`<button type="button" class="button" data-copy-resource="${esc(file)}">คัดลอกข้อความ</button>`:''}<button type="button" class="text-button" data-action="resources">กลับไปดูไฟล์ทั้งหมด</button></div>${['clinic-public-source.md','website-source-notes.md','source-example.md'].includes(file)?sourceLinks():''}${resource!==undefined?`<pre class="resource-preview">${esc(resource)}</pre>`:`<p class="empty-note">ดาวน์โหลดไฟล์เพื่อเปิดในเครื่อง</p>`}`);
  }
  function showGuide(){showDialog('คู่มือผู้สอน · พร้อมเดินคลาส',`<p class="dialog-intro">ผู้เรียน 6–10 คน · ผู้สอน 1 คน + ผู้ช่วย 1 คน</p><div class="button-row"><a class="button primary" href="./resources/instructor-guide.md" download>ดาวน์โหลดคู่มือ ↓</a><a class="button" href="./resources/slide-notes.md" download>โน้ต ${data.slides.length} สไลด์ ↓</a><button type="button" class="button" data-action="print">พิมพ์สไลด์ / PDF</button></div><section class="guide-section"><h3>เริ่มด้วยสไลด์ก่อนใช้คอม</h3><p>14:00–14:30 เปิด AI ใส่ซอส 20 หน้าให้ฟังและคุยด้วยวาจา ผู้เรียนเริ่มบัญชี โฟลเดอร์ และดาวน์โหลดเวลา 14:30 เรียนต่อเนื่องถึง 17:00 แล้วพักท้ายคลาส</p><a class="button small" href="./opening.html#slide/1">เปิดสไลด์ปูพื้นฐาน ↗</a></section><section class="guide-section"><h3>เดินคลาส 3 ชั่วโมง</h3>${agendaTable()}</section><section class="guide-section"><h3>ผลงานวันนี้</h3><ul><li>โพสต์ 1 ชิ้น + คำตอบแอดมิน 3 แบบ</li><li>Excel ที่จัดข้อมูลแล้ว + หน้าสรุป + คำสั่งใช้รอบหน้า</li></ul><p>หลังพื้นฐาน 30 นาทีจึงสาธิตและให้ทุกคนทำตาม ผู้ช่วยดูการเปิดและบันทึกไฟล์ตั้งแต่ 14:30</p></section><section class="guide-section"><h3>ภารกิจ AI Leader และนัดติดตาม</h3><p>7 วัน: ทำงานซ้ำเอง → พาเพื่อนทำได้ → เลือกซอสกลางของทีม คอลกลุ่ม 30 นาทีในอีก 1–2 สัปดาห์ เปิดหลักฐานจาก ai-leader-progress.md และแก้จุดติด</p><p>Advance เป็นคอร์สต่อยอดเต็มวันสำหรับนำ Source ขึ้น GitHub รวมติดตั้งและเตรียมเครื่อง มีค่าเรียนแยกจากนัดติดตามที่รวมไว้แล้ว</p></section><div class="button-row"><button type="button" class="button small" data-resource="excel-csv-guide.md">คู่มือ Excel / CSV</button><button type="button" class="button small" data-resource="answer-key.md">ผลลัพธ์อ้างอิง</button></div>`);}

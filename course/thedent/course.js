...
(() => {
  ...
  let accountPath = 'free';
  function promptText(p) { return (accountPath==='free'?data.freePromptPrefix:'')+p.text; }
  ...
  function download(name, text, type='application/octet-stream') { const url=URL.createObjectURL(new Blob([text],{type}));const a=document.createElement('a');a.href=url;a.download=name;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),3000); }
  ...
  function closingActions(slide=false) {
    return `<section class="closing-actions ${slide?'on-slide':''}" aria-label="จบคลาสและไปต่อ"><div class="closing-copy"><p class="eyebrow">3 นาที · ความคิดเห็นของคุณช่วยพัฒนาคลาส</p><h2>ประเมิน แล้วไปรับการ์ด</h2><p>ส่งแบบประเมิน → รับการ์ดบน TeamBook<br>เข้าห้องติดตามผล แล้วเริ่มภารกิจ 7 วัน</p><a class="button primary" href="./evaluation.html" target="_blank" rel="noopener">เริ่มแบบประเมิน →</a><a class="closing-join" href="https://www.teambook.me/join/?c=52113" target="_blank" rel="noopener noreferrer">เข้าห้องติดตามผล 52113 ↗</a></div><a class="closing-qr" href="./evaluation.html" target="_blank" rel="noopener" aria-label="เปิดแบบประเมิน หรือสแกน QR ด้วยโทรศัพท์"><img src="./followup-qr.svg" width="190" height="190" alt="QR เปิดแบบประเมินหลังคลาส The Dent"><span>สแกนด้วยโทรศัพท์ได้เลย</span></a></section>`;
  }
  function weekRules() {
    return `<section class="week-rules"><p class="eyebrow">7 DAYS · ONE SMALL WIN A DAY</p><h2>ทำหนึ่งอย่าง แล้วค่อยกด “ลงชื่อ”</h2><p>ในห้อง TeamBook กด <strong>ลงชื่อวันนี้ → ลงชื่อ</strong> วันละหนึ่งครั้ง หลังทำภารกิจแล้ว จะเล่าสั้น ๆ ว่า “วันนี้ทำอะไร ได้อะไร” ก็ได้</p><details><summary>ดูภารกิจครบ 7 วัน</summary><ol>${data.week.map(d=>`<li><span>วันที่ ${d.day}</span><div><strong>${esc(d.title)}</strong><p>${esc(d.action)}</p></div></li>`).join('')}</ol></details><p class="week-privacy">เก็บไฟล์งานในเครื่อง ไม่ต้องอัปโหลดข้อมูลลูกค้า · ถ้ายังไม่มี Export ใช้งานเอกสาร ภาพ หรือคอนเทนต์จาก Source สาธารณะได้</p></section>`;
  }
  function leaderHomework() {
    return `<section class="leader-homework" aria-labelledby="homework-heading"><div class="homework-heading"><div><p class="eyebrow">YOUR FIRST WEEK</p><h2 id="homework-heading">3 ภารกิจหลังคลาส</h2></div><p class="homework-progress" aria-live="polite" aria-atomic="true"><strong id="homework-count">${state.homework.size} / ${data.homework.length}</strong> ภารกิจที่ทำแล้ว</p></div><div class="homework-cards">${data.homework.map(m=>`<article class="homework-card ${state.homework.has(m.id)?'is-done':''}" data-homework-card="${esc(m.id)}"><div class="homework-card-top"><span class="homework-number">${number(m.level)}</span><span>${esc(m.when)}</span></div><h3>${esc(m.title)}</h3><p>${esc(m.action)}</p><details class="homework-evidence"><summary>เก็บผลงานอะไรไว้</summary><p>${esc(m.evidence)}</p></details><label class="homework-check"><input type="checkbox" data-homework="${esc(m.id)}" ${state.homework.has(m.id)?'checked':''} aria-label="ทำแล้ว: ${esc(m.title)}"><span>ทำภารกิจนี้แล้ว</span></label></article>`).join('')}</div><div class="homework-foot"><p>ติ๊กเมื่อทำแล้ว · จำความคืบหน้าในเบราว์เซอร์นี้<br>ลงชื่อใน TeamBook เมื่อทำภารกิจของวันนั้นแล้ว<br>เก็บไฟล์ไว้ในเครื่อง แล้วเปิดคุยวันติดตาม</p><button type="button" class="button small" data-resource="ai-leader-homework.md">ดูการบ้าน / ดาวน์โหลด ↓</button></div></section>`;
  }
  ...
  function freePath(m) {
    const f=data.freePaths[m.id];if(!f)return '';
    return `<section class="free-path" aria-label="ทางเรียนสำหรับบัญชี Free"><div class="free-path-head"><div><p class="eyebrow">FREE ACCOUNTS WELCOME</p><h2>ใช้ฟรี ก็ลงมือบทนี้ได้</h2></div><label>พรอมป์ที่คัดลอก<select id="account-path"><option value="free" ${accountPath==='free'?'selected':''}>ChatGPT / Claude Free · Chat</option><option value="desktop" ${accountPath==='desktop'?'selected':''}>Work / Cowork · ตามสิทธิ์บัญชี</option></select></label></div><div class="free-path-grid"><article><h3>ChatGPT Free</h3><p>${esc(f.chatgpt)}</p></article><article><h3>Claude Free</h3><p>${esc(f.claude)}</p></article></div><p class="free-path-task">${esc(f.task)}</p><details><summary>ถ้าชนโควตา หรือไม่มีไฟล์ให้ดาวน์โหลด</summary><p>เอกสาร: ขอ Markdown แล้วคัดลอกลง Notepad / TextEdit แบบ plain text บันทึก UTF-8 ชื่อไฟล์ลงท้าย .md<br>ตาราง: ขอ TSV คั่นด้วยแท็บ วางใน Excel ที่ A1 แล้ว Save As เป็น .xlsx; ต้องการ CSV ให้เลือก CSV UTF-8 แยกทีละชีต<br>โควตา: ลดเหลืองานเดียว ใช้ข้อความ Source สั้น ๆ แทนแนบไฟล์เมื่อแชตยังใช้ได้ หรือเก็บคำสั่งไว้รอรอบถัดไป บัญชีอีกตัวใช้ได้เมื่อมีสิทธิ์และโควตาของตัวเอง</p><p>Claude Free ขอสร้างไฟล์ได้ใน Chat หากไม่มีเครื่องมือ ให้ดู Settings → Capabilities → Code execution and file creation ส่วน Cowork ต้องใช้แผนเสียเงินบน Desktop</p></details></section>`;
  }
  ...
  document.addEventListener('click',async e=>{
    ...
    if(b.dataset.resetPrompt){const p=data.prompts.find(p=>p.id===b.dataset.resetPrompt);drafts.delete(p.id);document.querySelectorAll(`[data-prompt-id="${p.id}"]`).forEach(el=>el.value=promptText(p));notify('คืนข้อความตั้งต้นแล้ว');return;}

...
function addStyles() {
  ...
  style.textContent = `
    ...
    .c7-reward__visual{display:grid;gap:15px;align-items:start}
    .c7-reward__flip{position:relative;width:100%;aspect-ratio:63/88;border:0;padding:0;border-radius:18px;background:transparent;cursor:pointer;perspective:1300px;filter:drop-shadow(0 26px 28px rgb(0 0 0/.42));transform:rotate(-1.4deg);transition:transform .25s ease,filter .25s ease;-webkit-tap-highlight-color:transparent}
    ...
    .c7-reward__face{position:absolute;inset:0;overflow:hidden;border-radius:16px;backface-visibility:hidden;-webkit-backface-visibility:hidden;box-shadow:inset 0 0 0 1px rgb(255 255 255/.18)}
    .c7-reward__face--front{transform:rotateY(180deg)}
    .c7-reward__face svg{display:block;width:100%;height:100%;border-radius:inherit}
    ...
    .c7-reward__flip:not(.is-flipped)::after{content:"";position:absolute;inset:0;border-radius:18px;pointer-events:none;background:linear-gradient(110deg,transparent 28%,rgb(255 255 255/.13) 46%,transparent 62%);transform:translateX(-120%);animation:c7RewardShine 2.8s ease-in-out infinite}
    .c7-reward__flavor{margin:0;padding:15px 16px;border:1px solid rgb(234 208 140/.25);border-radius:15px;color:rgb(255 255 255/.82);background:rgb(255 255 255/.055);font:500 14px/1.75 system-ui;quotes:"“" "”";animation:c7RewardStory .45s .08s ease both}
    .c7-reward__flavor::before{content:open-quote;color:#ead08c;font:800 25px/0 "Bai Jamjuree",system-ui;margin-right:3px;vertical-align:-5px}.c7-reward__flavor::after{content:close-quote;color:#ead08c}
    ...
    @media(max-width:620px){.c7-reward{align-items:end;padding:10px}.c7-reward__panel{grid-template-columns:116px minmax(0,1fr);gap:17px;border-radius:24px;padding:20px}.c7-reward__copy h2{font-size:27px}.c7-reward__hint{font-size:12.5px}.c7-reward__flavor{grid-column:1/-1;font-size:13px}.c7-reward__visual{display:contents}.c7-reward__flip{grid-column:1;grid-row:1;align-self:center}.c7-reward__copy{grid-column:2;grid-row:1}.c7-reward__flavor{grid-row:2}.c7-reward__meter,.c7-reward__unlock,.c7-reward__set-note,.c7-reward__actions{grid-column:1/-1}.c7-reward__actions{display:grid;grid-template-columns:1fr 1fr}.c7-reward__btn{padding-inline:10px}.c7-reward__face--back::after{font-size:7px;bottom:7px;padding:5px 7px}}
  ...
  overlay.innerHTML = `
    <section class="c7-reward__panel">
      <div class="c7-reward__visual">
        ...
        <blockquote class="c7-reward__flavor" data-flavor hidden>${card.story || 'การ์ดใบนี้ถูกเพิ่มลงใน FIRST HAND Collection ของคุณแล้ว'}</blockquote>
      </div>

...
- Facebook/Open Graph and Twitter use `assets/airova-share-original-v9.png`, a 2400 × 1260 lossless PNG restoring the user’s preferred first campaign. The original portrait, oversized Thai copy, saturated colors and continuous neon ribbons remain together as one image. Only the incorrect brand band and a new MCP module are composed separately.
...
- V9 preserves the supplied early campaign directly. A surgical built-in imagegen brand erase supplies only a small top-left patch; canonical logos replace the incorrect marks. The bottom-right MCP Ready badge uses a dark glass panel, cut corners, a cyan/violet rim, and original ChatGPT/Claude marks. Editable composition: `_source/share-preview-v9.html`; prompt: `_source/share-v9-prompt.md`. No portrait mask or cutout is used.

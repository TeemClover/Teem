/* myClover · AWAKEN dialogue skip detector v2
   - เปรียบเทียบภาษาลัดกับรูปแบบ Asking / Doing จากงานวิจัยสาธารณะ
   - นับ Active Time จริง หยุดเมื่อสลับแอปหรือซ่อนหน้า
   - คน Skip All Dialogue ยัง Clear ได้ แต่หีบแรกมีเกลือ 1 ขวด
*/

const RUN_SESSION_KEY = 'mc_awaken_run_session_v3';
const RUN_REPORT_KEY = 'mc_awaken_run_reported_v3';
...
function classifyRun() {
  ...
  const noExploration = explored === 0 && !notebook;
  const hardSkip = first90Ms < 16000 && run.maxProgress >= .88 && noExploration;
  const emptySprint = activeMs < 42000 && run.maxProgress >= .95 && noExploration;
  const mouseWheelWarp = activeMs < 52000 && run.rapidBursts >= 4 && noExploration;
...
function boot() {
  if (!isBossPage() || document.documentElement.dataset.awakenRushV2 === '1') return;
  document.documentElement.dataset.awakenRushV2 = '1';
  injectStyles();

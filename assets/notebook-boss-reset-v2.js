...
async function pushResetToCloud(){try{const response=await fetch('/api/progress',{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({resetScope:'dungeon',progress:resetPayload()})});if(response.ok||response.status===401){try{localStorage.removeItem(RESET_PENDING_KEY)}catch{}return true}}catch{}return false}

...
function styles(){
 ...
 .course-learning .cl-launch,.course-learning .cl-completion,.course-learning .cl-rail,.course-learning .cl-steps,.course-learning .cl-introduction{box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Noto Sans Thai","Thonburi",sans-serif;line-height:1.6}
 ...
 .cl-launch .cl-launch-grid{display:grid;grid-template-columns:minmax(0,1fr) 150px;gap:22px;align-items:center}
 ...
 .cl-launch .cl-art{display:block;width:100%;padding:0;border:1px solid #89a578;border-radius:14px;overflow:hidden;background:#fff;cursor:pointer;color:#254831;box-shadow:0 8px 22px #061c1355;transform:rotate(2deg);transition:transform .18s}
 .cl-launch .cl-art:hover{transform:rotate(0)}.cl-launch .cl-art img{display:block;width:100%;height:auto;max-width:100%}.cl-launch .cl-art span{display:block;font-family:inherit;font-size:10px;font-weight:700;line-height:1.5;padding:5px 4px;text-align:center;background:#eff2e6}
 ...
 .cl-introduction{margin:10px 0 20px;padding:0;border:1px solid #d4ddcf;border-radius:14px;background:#f0f2e9;color:#31553e;overflow:hidden}.cl-introduction>summary{padding:12px 17px;cursor:pointer;font-size:13px;font-weight:750;list-style:revert}.cl-introduction[open]>summary{border-bottom:1px solid #d4ddcf}
 .course-learning .cl-introduction>.head,.course-learning .cl-introduction>.hero{margin:0!important;border:0!important;border-radius:0!important;box-shadow:none!important}
 ...
 @media(max-width:700px){.cl-rail{gap:5px}.cl-rail a{min-height:38px}.cl-rail a span{display:none}.cl-launch{padding:22px 19px!important;border-radius:18px!important}.cl-launch .cl-launch-grid{grid-template-columns:minmax(0,1fr) 90px;gap:12px;align-items:start}.cl-launch .cl-title{font-size:27px!important}.cl-launch .cl-eyebrow{font-size:10px;letter-spacing:.1em}.cl-launch .cl-outcome{font-size:14px!important}.cl-launch .cl-need{grid-column:1/-1}.cl-launch .cl-actions{margin-top:20px;gap:8px}.cl-launch .cl-actions a{font-size:13px;padding:11px 14px}.cl-launch .cl-example{flex:1 1 140px}.cl-launch .cl-start{flex:1 1 180px}.cl-steps a{font-size:11px;gap:4px}.cl-completion{padding:18px!important}.cl-completion h3{font-size:21px!important}}
 @media(prefers-reduced-motion:reduce){.cl-launch .cl-art,.cl-launch .cl-track i{transition:none}.cl-launch .cl-art{transform:none}}
...
function boot(){
 ...
 const intro=document.createElement('details');intro.className='cl-introduction';intro.id='cl-introduction';
 const summary=document.createElement('summary');summary.textContent='ดูแผนภาพและแนวคิดของบทนี้';intro.appendChild(summary);hero.before(intro);intro.appendChild(hero);
 const art=hero.querySelector('.classroom-lesson-header-image img');
 ...
 if(art){const button=document.createElement('button');button.type='button';button.className='cl-art';button.setAttribute('aria-controls',intro.id);button.setAttribute('aria-expanded','false');const img=document.createElement('img');img.src=art.getAttribute('src');img.alt='แผนภาพบทที่ '+(index+1);img.width=150;img.height=150;img.decoding='async';button.appendChild(img);const label=document.createElement('span');label.textContent='ดูแผนภาพ ↗';button.appendChild(label);launch.querySelector('.cl-launch-grid').appendChild(button);button.addEventListener('click',()=>{intro.open=!intro.open;button.setAttribute('aria-expanded',String(intro.open));if(intro.open)goTo(intro);});intro.addEventListener('toggle',()=>button.setAttribute('aria-expanded',String(intro.open)));}
 ...
 const targets=[lesson.work,lesson.check,'#cl-completion'];
 ['เริ่มลงมือ','ชิมผลงาน','เก็บงานไปต่อ'].forEach((text,i)=>{const a=document.createElement('a');a.href=targets[i];a.dataset.step=String(i);a.innerHTML='<b>'+String(i+1).padStart(2,'0')+'</b> '+text;steps.appendChild(a);});
 intro.before(rail,launch,steps);

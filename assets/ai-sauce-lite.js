...
(function(){
  ...
  function markLesson(){
    try{
      var t=window.MC_QUEST&&window.MC_QUEST.track('learn');
      if(t) t.mark(slug);
    }catch(e){}
  ...
  if(finish){
    finish.addEventListener('click',function(){
      ...
      if(finish.hasAttribute('data-boss-trigger')){
  ...
  document.querySelectorAll('[data-download-html]').forEach(function(button){
    button.addEventListener('click',function(){
      var id=button.getAttribute('data-download-html'), template=document.getElementById(id);
      if(!template) return;

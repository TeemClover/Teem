/* ⚠️ ไฟล์นี้ถูกโหลดเป็น type="module" แล้ว (ดู collection/index.html)
   กติกาว่าอะไรปลดแล้วย้ายไปอยู่ที่ assets/achievement-state.js เพื่อให้
   ตัวรายงานสถิติอ่านชุดเดียวกันได้ทุกหน้า ไม่ใช่เฉพาะตอนเปิดหน้านี้ */
import { achievementState } from '/assets/achievement-state.js';
(function(){
'use strict';
var ST=achievementState();
function on(id){return ST[id]===true}
...
var lessonItems=LEARN.map(function(slug,i){return item({id:'lesson-'+(i+1),kind:'FREE LESSON '+(i+1),name:LEARN_NAME[i],icon:['✨','🖼️','🎬','📚','🧠','🌐'][i],on:on('lesson-'+(i+1)),hint:'เรียนบทที่ '+(i+1)+' จนจบ',go:'../classroom/'+slug+'.html',image:'../img/lesson-'+(i+1)+'.jpg'})});
...
item({id:'home',kind:'FIRST STEP',name:'เริ่มเดินในบ้าน myClover',icon:'🍀',on:on('home'),hint:'เริ่มอ่าน เรียน เลือกสาย หรือเล่นเกมอย่างใดอย่างหนึ่ง',go:'../',image:'../img/og-home.jpg'}),
item({id:'path',kind:'COMPASS',name:'เลือกสายของตัวเอง',icon:'🧭',on:on('path'),hint:'เลือกพลัง 1 จาก 4 สาย และเปลี่ยนได้ตลอด',go:'../paths/',image:'../img/og-paths.jpg'}),
item({id:'club',kind:'ROOM VISITED',name:'myClover Club',icon:'🍜',on:on('club'),hint:'เข้าไปเยี่ยม Club 1 ครั้ง',go:'../club/',image:'../img/og-club.jpg'}),
item({id:'resume',kind:'ROOM UNLOCK',name:'Smart Resume',icon:'🧾',on:on('resume'),unlockHint:'อ่าน Forge ครบ 7 ตอน',unlockedHint:'สำรวจ Career × Lifestyle และผลงานที่สร้างจริง',go:'../resume/',image:'../img/og-resume.jpg'}),
...
item({id:'blacksmith',kind:'SIGIL',name:'ช่างตีเหล็ก',icon:'⚒️',on:on('blacksmith'),hint:'อ่าน Forge ครบ 7 ตอน',go:'../forge/',image:'../forge/img/07-thumb.webp?v=20260802-sigil'}),
...
item({id:'notebook-restored',kind:'SECRET',name:'สมุดที่ซ่อมแล้ว',lockedName:'???',icon:'📓',on:on('notebook-restored'),hint:'ใช้ RESTORE ซ่อมหน้าที่เสียหาย',go:'../classroom/awaken/notebook/',image:'../classroom/awaken/notebook/img/nb-01.jpg',imageStyle:'object-position:center top',secret:true}),

...
const BEATS = Object.freeze({
  opening: ["ผมทีม จะช่วยพาลองทีละอย่างนะ เริ่มจากคนหนึ่งคน แล้วมาดูว่าเราจะสร้างอะไรต่อได้", "นี่คือเกมจำลอง วันในเกมผ่านได้ทันที ไม่ต้องมีอุปกรณ์หรือซื้ออะไรก่อนเล่น"],
  pre_day0_band: ["เริ่มจากลองดูแลตัวเองในเกมก่อนนะ กดใส่สายรัด แล้วดูว่ามันช่วยเล่าอะไร", "Xircle Band ช่วยดูการขยับและสัญญาณระหว่างวัน"],
  ...
  pre_day0_scanning: ["รอแป๊บเดียว ข้อมูลจำลองกำลังมา"],
  pre_day0_summary: ["ได้จุดเริ่มต้นแล้ว ลองเลือกสิ่งเล็ก ๆ ที่ทำต่อได้ทุกวันกัน"],
  pre_montage: ["วันในเกมกำลังผ่านไป ลองดูว่าสิ่งที่ทำซ้ำจะทิ้งร่องรอยอะไรไว้"],
  ...
  pre_day14_scale: ["ผ่านมา 14 วันในเกมแล้ว คิดว่าตัวเลขทุกตัวจะเปลี่ยนไปพร้อมกันไหม?"],
  ...
  pre_day28_scale: ["ครบ 28 วันจำลองแล้ว มาดูว่าควรเก็บอะไรไว้ทำต่อ"],
  ...
  certification_ceremony: ["ผ่านแล้วนะ เก็บสิ่งที่เรียนไปลองใช้กับคนแรกในเกมกัน", "ใบรับรองนี้เป็นส่วนหนึ่งของเกม"],
  certified: ["โต๊ะยังว่างอยู่ แต่เราเริ่มได้แล้ว ลองทำให้คนหนึ่งคนอยากกลับมาหาเรา", "พลังงาน 28 แต้มคือเวลาทำงานในแต่ละเดือนของเกม"],
  ...
  m1_recommendation: ["เล่าแผนให้ชัด แล้วให้เขาเลือกเองว่าจะเริ่มหรือยัง"],
  ...
  m1_candidate: ["เขาอยากช่วยคนอื่นบ้างแล้ว เรามาช่วยให้เขาทำได้ด้วยตัวเองกัน", "X-VISOR คือบทบาทผู้ดูแล ต้องเรียนและผ่านการรับรองในเกมก่อน"],
...
function selectStoryBeat(state = {}, content = {}, context = {}) {
  ...
  if (state.stage === "live_running") return mentor("live-running", "คุยกับคนที่พร้อมได้หลายคนในครั้งเดียว แล้วฟังว่าใครอยากเริ่มหรือขอเวลา");
  ...
  if (freshLive && Number.isFinite(live.attempted) && Number.isFinite(live.sales)) {
    ...
    return mentor(`live-result:${live.id}`, live.attempted ? `คุยผ่าน Live ${live.attempted} คน มี ${live.sales} คนเลือกเริ่ม${waiting ? ` และ ${waiting} คนขอเวลา` : ""}` : "รอบนี้ยังไม่มีคนพร้อมคุยแผน ลองกลับไปฟังบริบทของเขาก่อน", live.sales ? "คนที่เริ่มแผนยังต้องรับการดูแลต่อจากวันแรก" : "ยังไม่มียอดขายจากรอบนี้ ให้เวลากับคนที่ยังขอคิด");
  }
  ...
  if (state.campaignComplete && !state.organizationMode && context.surface !== "month-summary") {
    return mentor("month12", "ผ่านปีแรกแล้ว ปีต่อไปมาดูว่าลูกค้ากับทีมที่ดูแลไว้จะเดินต่ออย่างไร", "คะแนนปีแรกเก็บไว้ที่นี่ และคุณเล่นต่อได้ถึงเดือน 24", "proud");
  }
  ...
  if (!state.organizationMode && !state.career?.xgenExamPassed && (state.career?.xgenQualifiedSingleMonth || state.career?.xgenQualificationRule === "single-month")) {
    return mentor("xgen-ready", "ผลงานเดือนหนึ่งถึงเกณฑ์ XGEN แล้ว เหลือทบทวนและสอบให้ผ่านก่อนเปิดช่อง ③", "เกณฑ์คือ 3,000,000 XV ในเดือนเดียว XV เป็นหน่วยผลงานในเกม ไม่ใช่เงินบาท");
  }
  ...
  if (["CHOOSE_MANAGEMENT_ROUTINE", "OFFER_PROSPECT", "MAKE_OFFER"].includes(context.event) && context.previousState && state.stage === "management") {
    ...
    if (person?.journey === "waiting" && Number(person.nextOfferMonth) > Number(state.month)) return mentor(`offer-waiting:${id}:${person.nextOfferMonth}`, `${person.name || "เขา"}ขอเวลาคิดก่อน ความสัมพันธ์ยังอยู่`, `กลับไปคุยเรื่องแผนได้ในเดือน ${person.nextOfferMonth}`);
  }

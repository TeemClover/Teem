...
export function getMonthlyHistory(state = {}) {
  ...
  return [...byMonth.values()].sort((a, b) => a.month - b.month).map(record => {
    ...
    return {
      ...
      recurringBase: finite(growth.recurringBase),
      customerBase: finite(growth.customerBase),
      directMemberBase: finite(growth.directMemberBase),
      loyalCustomers: finite(growth.loyalCustomers),
      recurringScope: "personal-direct",
...
export const GROWTH_METRICS = [
  ...
  { key: "recurringBase", label: "ฐานลูกค้าและ X-VISOR สายตรงสะสม", unit: "คน", scope: "recurringScope" },
  { key: "repeatCustomers", label: "ซื้อรอบใหม่", unit: "คน", scope: "customerScope" },
  { key: "activeCustomers", label: "คนที่ใช้อยู่", unit: "คน", scope: "customerScope" },

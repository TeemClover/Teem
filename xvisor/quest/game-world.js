...
import { drawMentorSprite } from "./game-mentor-art.js";
import { createSceneAtlas } from "./game-scene-atlas.js";
...
export function createWorldRenderer(canvas, getSnapshot) {
  ...
  const sceneAtlas = createSceneAtlas(() => { backgrounds.clear(); invalidate(); });
  let atlasBackground = false;
...
function eventCardForScene(scene, stageAge, organizationMode) {
  ...
  const cards = {
    "live-studio": { kicker: "LIVE STUDIO", title: "LIVE กับทีมและเอโกะ", detail: `กำลังพูดคุยกับ ${state.pendingLive?.targetIds?.length || 0} คนที่พร้อมฟัง · รอดูผลหลังจบ Live`, tone: "academy" },
...
function drawRoom(theme = "office") {
  atlasBackground = sceneAtlas.draw(context, "office");
  if(!atlasBackground)background(`room:${theme}`, layer => layer.room(theme));
}
...
function drawOfficeGrowth(scene) {
  // The atlas already contains furniture and wall displays. Growth is still
  // visible through the number of customers/team members in the foreground.
  if(atlasBackground)return;
...
function drawCharacterAtFeet(x,footY,palette=playerPalette,options={}) {
  if(palette.characterId) {
    art.shadow(x+16,footY+1,19,4);
    const bob=options.idle&&!reducedMotion.matches?Math.sin(visualTime/850+x)*.4:0;
    if(drawMentorSprite(context,palette.characterId,x+16,footY+2+bob,86,invalidate))return;
  }
...
function drawActionScene(moment, progress) {
  ...
  const mentor = ["care","routine","onboarding","followup"].includes(group)?MENTOR_PALETTES.ako:MENTOR_PALETTES.teem;
  const npc = moment.person?.appearance || mentor;
  ...
  if(group==="welcome") {
    ...
    drawCharacterAtFeet(279,190,MENTOR_PALETTES.ako,{pose:"welcome",direction:"left",idle:true});
  } else if(group==="care") {
    ...
    drawCharacterAtFeet(160,192,moment.companion?.appearance||npc,{pose:"present",direction:"right",band:Boolean(moment.companion?.band),gesture});
    ...
    if(variant==="people"||variant==="candidate") {
      ...
      drawCharacterAtFeet(281,192,MENTOR_PALETTES.teem,{pose:"read",direction:"left",notebook:true});
      drawLaptop(202,129,true);
      if(variant==="care")drawDataPanel(43,62,false);
      else if(variant==="leadership")drawWhiteboard(37,61);
    ...
    drawCharacterAtFeet(87,192,MENTOR_PALETTES.teem,{pose:"read",direction:"right",notebook:true});
    ...
    drawCharacterAtFeet(311,192,MENTOR_PALETTES.ako,{pose:"read",direction:"left",notebook:true});
    if(variant==="open")art.line(99,123,119,123,"#83aa8c",1.2);
  } else if(group==="live")drawLiveScene(phase*1000);
}
function drawLocation(location) {
  atlasBackground=sceneAtlas.draw(context,location||"office");
  if(atlasBackground)return;
...
function drawLiveScene(age) {
  ...
  drawCharacterAtFeet(230,190,MENTOR_PALETTES.ako,{pose:"present",direction:"left",idle:true});
  drawCharacterAtFeet(72,181,MENTOR_PALETTES.teem,{pose:"read",direction:"right",idle:true});
...
function drawScene(time) {
  ...
  const sceneDescription = moment ? `${moment.title}${moment.person?.name?`กับ ${moment.person.name}`:""} · ${moment.detail}` : scene==="live-studio" ? "ผู้เล่นกับทีมและเอโกะกำลัง LIVE คุยกับคนที่พร้อมในสตูดิโอ" : organizationMode?.kind === "travel" ? `ทีมรับรางวัลท่องเที่ยวที่ ${organizationMode.report.trip.destination}` : scene === "the-xircle" || organizationMode?.kind === "xircle" ? "ทีมร่วมแคมป์ The Xircle ใต้แสงดาว" : exam ? "ห้องสอบ Xcademy" : "ฉากสำนักงานและทีม X-VISOR";
  ...
  if (moment) {
    ...
    drawCharacterAtFeet(71,181,MENTOR_PALETTES.teem,{direction:"right"});
    ...
    drawCharacterAtFeet(75,181,content.portrait==="ako"||content.speaker==="เอโกะ"?MENTOR_PALETTES.ako:MENTOR_PALETTES.teem,{direction:"right"});
    ...
    if(["pre_band","pre_summary","pre_abcd"].includes(scene))drawCharacterAtFeet(185,182,content.portrait==="ako"||content.speaker==="เอโกะ"?MENTOR_PALETTES.ako:MENTOR_PALETTES.teem,{pose:"read",notebook:true,direction:"left",idle:true});
    ...
    if (scene === "pre_montage") {
      ...
      if(scene==="practice_data")drawCharacterAtFeet(261,184,MENTOR_PALETTES.ako,{direction:"left",pose:"welcome"});
      else drawSittingCharacter(254, 159, npc, "left");
    ...
    drawCharacterAtFeet(127,184,playerPalette,{idle:true,direction:"right",pose:"listen"});
    drawCharacterAtFeet(205,184,MENTOR_PALETTES.teem,{idle:true,direction:"left",pose:"read",notebook:true});
    drawCharacterAtFeet(271,188,MENTOR_PALETTES.ako,{idle:true,direction:"left",pose:"welcome"});
    ...
    if(content.portrait==="ako"||content.speaker==="เอโกะ")drawCharacterAtFeet(133,183,MENTOR_PALETTES.ako,{direction:"right"});
    ...
    drawCharacterAtFeet(98,134,MENTOR_PALETTES.ako,{pose:"welcome",direction:"right"});
    drawCharacterAtFeet(188,134,MENTOR_PALETTES.teem,{pose:"present",direction:"left"});
    ...
    const crowd = [playerPalette, npc, ...state.team.slice(0, 2).map((member) => member.appearance || npc)].slice(0, 4);
    const seats=[38,104,248,314];
    crowd.forEach((palette, index) => drawSittingCharacter(seats[index],177,palette,index<2?"right":"left",{pose:"listen",band:palette===playerPalette||Boolean(state.team[index-2])}));

...
function routineContent(state, management = false) {
  ...
  const labels = {
    ...
    fit: ["เริ่มแผนที่พอดี", "เลือกตัวช่วยที่เหมาะ แล้วคุยแผนนี้ครั้งเดียว"],
...
function getStageContent(state) {
  ...
  const copy = {
    ...
    [STAGES.M1_RECOMMENDATION]: {
      ...
      speaker: "ทีม",
      dialogue: "อธิบายสิ่งที่จะลองกับวิธีติดตาม แล้วให้เขาตัดสินใจ",
...
function nameTutorialActions(state, content) {
  ...
  const labels = {
    ...
    [EVENTS3.MAKE_OFFER]: `📁 คุยแผนกับ ${name}`,
...
function getStageContent3(state) {
  if (state.stage === STAGES.LIVE_RUNNING) {
    return finish(state, {
      ...
      eyebrow: `Live · เดือน ${state.month}`, title: "คุยพร้อมกัน ผ่าน Live",
      reason: "ให้แต่ละคนฟังและเลือกตามความพร้อม",
      speaker: "ทีม", dialogue: "คุยกับคนที่พร้อมได้หลายคนในครั้งเดียว แล้วฟังว่าใครอยากเริ่มหรือขอเวลา",
      status: "live", actions: []
    });
  }

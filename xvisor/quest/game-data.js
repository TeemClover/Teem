...
const QUICK_ACTION_CONTEXT = new Map([
  ...
  [base.EVENTS.OFFER_PROSPECT, ["people", "คุยแผน", "ทบทวนแผนที่วางร่วมกัน แล้วให้เขาตัดสินใจเมื่อพร้อม"]],
  [base.EVENTS.FOLLOW_UP_DECISION, ["people", "ต่อบทสนทนา", "ครบเวลาที่ขอคิดแล้ว กลับไปฟังข้อสงสัยและความพร้อม"]],
  [base.EVENTS.FAST_TRACK_FULL_START, ["people", "คุยแผน", "เมื่อเริ่มครบชุดแล้ว ยังต้องดูแลให้เกิดผลลัพธ์จริง"]],
  ...
  [base.EVENTS.RUN_LIVE, ["growth", "ไลฟ์คุยแผน", "คุยกับคนที่พร้อมได้สูงสุด 3 คน แต่ละคนเลือกเองว่าจะเริ่มแผนหรือขอเวลา"]],
...
function managementAlternatives(state) {
  ...
  if (live.available) actions.push({ type: "live", event: base.EVENTS.RUN_LIVE, label: `📹 ไลฟ์คุยแผน · คุยได้ ${Math.min(live.capacity, live.eligibleCount)} คน`, cost: live.cost, score: 140 + Math.min(live.capacity, live.eligibleCount) * 5, category: "growth", categoryLabel: "ไลฟ์คุยแผน", reason: live.reason });

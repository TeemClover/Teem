...
function refreshMissions(state) {
  ...
  state.prospects.forEach((person) => {
    ...
    if (person.journey === "baseline") missions.push(makeMission(person.routinePlan ? "offer" : "routine", person.id, `${person.name} · ${person.routinePlan ? "คุยแผนที่เลือกไว้" : "วาง Routine จากข้อมูล"}`));
...
function buildPersonAction({ event, target, state, reason = "", expectedOutcome = "" } = {}) {
  ...
  const labels = {
    ...
    [EVENTS3.OFFER_PROSPECT]: `💬 คุยแผนกับ ${name}`,
...
function hasRoutineContext(person) {
  // The journey is also evidence for older saves that did not store `measured`.
  return Boolean(person?.consent && ["baseline", "recommendation"].includes(person.journey));
}
function getRoutineChoices(state, personOrId = state?.selectedPersonId) {
  const person = typeof personOrId === "object" && personOrId ? personOrId : (state?.prospects || []).find((item) => item.id === personOrId);
  ...
  return [
    ...
    },
    {
      ...
      reason: blocked || (!fitReady && !tutorial ? "ความไว้ใจและความพร้อมยังไม่พอสำหรับเริ่มแผนสินค้า" : "ใช้ตัวช่วยเฉพาะที่ตรงกับสิ่งที่เขาอยากเปลี่ยน เลือกแล้วคุยแผนนี้ครั้งเดียว"),
...
function chooseAndOfferRoutine(state, event, payload) {
  ...
  const updated = {
    ...person, journey: "recommendation", status: "พร้อมคุยแผนที่เลือก",
...
function recoverRoutineConsent(person) {
  // Older saves can omit consent after the completed Baseline checkpoint. An
  // explicit false remains false: never reinterpret a refusal as permission.
  return person && person.consent == null && person.measured && ["baseline", "recommendation"].includes(person.journey) ? { ...person, consent: true } : person;
}
...
function resumeRoutineOffer(state, event, payload) {
  ...
  const original = (state.prospects || []).find((item) => item.id === (tutorial ? state.selectedPersonId : payload.id));
  ...
  if (!hasRoutineContext(person)) return refreshMissions({
    ...state, stage: tutorial ? STAGES.M1_DISCOVERY : STAGES.MANAGEMENT,
    prospects: updatePerson(state.prospects, person.id, (item) => ({ ...item, journey: "discovery", status: "แผนเดิมยังอยู่ · ขออนุญาตดูข้อมูลก่อนคุยต่อ" })),
    lastEvent: "ROUTINE_CONSENT_REQUIRED", lastMessage: `${person.name} ยังไม่ได้อนุญาตให้ดูข้อมูล · ขออนุญาตก่อน แล้วคุยแผนเดิมต่อได้โดยไม่ต้องเลือกใหม่`, updatedAt: Date.now()
  });
...
function liveEligible(state, person) {
  if (!hasRoutineContext(person) || person.careOnly || person.routinePlan?.id === "control" || person.routinePlan?.id === "all") return false;
...
function reduceGame7(currentState, event, payload = {}) {
  ...
  if (event === EVENTS.RUN_LIVE) {
    ...
    return { ...state, stage: STAGES.LIVE_RUNNING, pendingLive: { id: `live-${state.encounters.seed}-${state.month}`, month: Number(state.month), startedAt: Date.now(), targetIds: readiness.targetIds, cost: readiness.cost }, sceneReport: { kind: "live-running" }, lastEvent: event, lastMessage: "กำลัง LIVE คุยแผนกับคนที่พร้อม แต่ละคนยังเลือกได้ว่าจะเริ่มหรือขอเวลา", updatedAt: Date.now() };
  }
...
function parseSavedState7(raw) {
  ...
  return state ? { ...state, prospects: (state.prospects || []).map(recoverRoutineConsent) } : state;
}
export {
